- **A view whose loop items change on every render stops paying for the loop render cache (#3071).**
  ...
  under 20%, it turns the cache off for the rest of its lifetime. Renders with
  no cacheable loop don't count, and a render that hits resets the streak, so a
  reorder-heavy list keeps its cache. Output is byte-identical either way. A
