- **Free-threaded CPython: the first simultaneous page loads after a start no
  ...
  payloads, and the mount frame's `cache_config`/`handler_config` — whose
  extractor swallowed the error and shipped a mount frame without the client
  rate-limit config. An AST gate fails any new request-path `dir()` or live
  namespace iteration. Regression cases in
