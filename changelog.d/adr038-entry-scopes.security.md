- **Explicit-view errors on the HTTP, SSE and WebSocket entry points (ADR-038
  ...
  with its message. The HTTP GET (including `streaming_render`), the SSE stream
  GET and SSE navigation now answer with the project's generic 500 page
  instead of Django's DEBUG technical page, and log a static line.
