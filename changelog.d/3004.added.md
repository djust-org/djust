- **Scoped server push: `push_to_view(..., scope=...)` and
  ...
  - A view now sets `self.push_scope = room` (a str, an int, or a list of
    them), and `push_to_view` / `apush_to_view` accept `scope=` to reach only
    those sessions.
  - Reassigning `push_scope` in a handler moves the session at the end of the
    turn.
  ...
  9 regression cases in `python/djust/tests/test_scoped_push_3004.py`.
