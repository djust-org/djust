...
`clear_draft()` takes effect at once over a live connection (it pushes a `djust:draft-clear` event to the page), and also through the render that follows it: the client removes the saved draft when it applies an update carrying `data-draft-clear`, whether that is a page load or a patch. Each call arms it again. (Before 1.2.1 the client checked the flag only at page load, and a second `clear_draft()` in a session was ignored; #2971.)
