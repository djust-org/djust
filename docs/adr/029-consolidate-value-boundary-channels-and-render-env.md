...
### Gate 4: Thread-Local Characterization and Scope (Smell on Default Path; Verified Bug on Actor Path, #2741)
...
* On the **actor path**, the smell is an **observed behavioral bug** (#2741), verified by `actor_render_on_a_worker_thread_reads_resolve_lazy_default_not_config_2741` in `crates/djust_live/src/actors/view.rs` (PR #2751): with `set_resolve_lazy(false)` on the calling thread, a `ViewActor` render on a single-worker multi-thread runtime returned the lazy answer, the worker's own `resolve_lazy()` read `true`, and thread distinctness was asserted (`worker_tid != setter_tid`). The in-test gate-off — setting the flag on the worker via `on_thread_start` — flips the render, so the probe observes the flag rather than assuming it:
  - In `crates/djust_live/src/actors/session.rs:259`, `tokio::spawn(view_actor.run())` (after `ViewActor::with_template_and_dirs` at `:255`) drives rendering from a tokio worker thread. (`supervisor.rs:111` spawns the enclosing `SessionActor`; `view.rs:560` spawns the child `ComponentActor`, which renders through a bare `Context::from_dict` at `component.rs:378` and so also skips per-view config.)
