...
### Gate 4: Thread-Local Characterization and Scope (Smell on Default Path; Probable — Unverified — Bug on Actor Path, #2741)
...
* On the **actor path**, the smell is *expected* to manifest as a behavioral bug (#2741). **This is inferred from reading the dispatch path and has not yet been exercised** — the map records the actor path as read-not-run (`VALUE_BOUNDARY.md` §6.7). The §6 actor-parity test is what confirms or refutes it; until it lands, this gate treats the actor-path bug as probable, not observed:
