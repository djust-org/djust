...
1. **A `RenderEnv` value** in `djust_core` holding what survives ADR-027 Step 5: timezone, the two number formats, the l10n stack. (`RESOLVE_LAZY` rode on it only until Step 5 deleted the flag — #2628; `CONVERSION_DEPTH`, the loop cache and the registry namespace are per-render scratch, not configuration, and stay as they are.)
...
The eight `resolve_lazy()` readers (six conversion-time: `stated_len_is_too_large_to_enumerate`, `list_repr_is_this_objects_own_spelling`, `opaque_gate` ×3, `opaque_value`; two render-time: `context.rs:1800`, `renderer.rs:5461`) kept reading the cell until ADR-027 Step 5 (#2628) deleted all eight. Nothing in this ADR threads an environment through `FromPyObject` or the `extract::<Value>()` sites; since Step 5, conversion needs no environment at all.

## 3. Sequencing against ADR-027 Step 5

ADR-027 `:435` settles that Step 5 deletes `template_resolve_lazy` **and** its enumeration arms together. (It said "in the first minor after a full release has soaked (1.3.0)"; the maintainer waived the soak on 2026-09-10 and Step 5 shipped in 1.2.0 as #2628. `:443`'s "the flag stays one release as a kill-switch, removal at 2.0" was superseded and is now tidied in the same PR.)

1. **Now, as a bugfix (#2741) — done (PR #2772):** the field, the actor entries applying it, the `ComponentActor` fix, the entry guard. #2751's probe is written to go red at this point — flip its expectation, do not delete it; its thread-distinctness harness is the proof the fix reached the worker.
2. **Step 5 as written — done (#2628, 1.2.0):** the flag is deleted from the field. The field shrank; no signature unwound. The #2751 probe and its `SessionActor` sibling now observe the number format (#2221) instead of the deleted flag — the thread-distinctness harness is unchanged.

## 4. Non-goals

- **No channel consolidation.** The sidecar's narrowing to top-level models, the alias-fallback deletion, and `_protect_sidecar_tree`'s removal were ADR-027 Step 5 (`:439`) — and Step 5's implementation (#2628) found all three to be Django-parity mechanisms for handle-less values (models, request-scoped keys) and kept them; see ADR-027's "what implementing it corrected" paragraph. `Context::aliases` **stays**: `is_safe` (`crates/djust_core/src/context.rs:1202`) resolves a `mark_safe` grant through it, and deleting it would regress the #2375/#2334 XSS-parity fix. Carrying a model as an `Encoded` (a live handle on a model) would reverse ADR-027 (b)(1) and needs its own ADR arguing against that ADR's table on the client JSON payload, the msgpack rolling-deploy pin and container change detection.
...
5. **Limit, not a correction:** per-event re-conversion on the worker (`get_context_data` → `extract::<Value>` in `crates/djust_live/src/actors/view.rs`) still read the worker's `resolve_lazy` cell. Only the mount-time conversion runs on the pushing thread. Closed by ADR-027 Step 5 (#2628): conversion no longer reads any environment cell.
