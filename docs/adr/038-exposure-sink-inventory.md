...
| Rust render state and render caches | mixins/rust_bridge.py: _sync_state_to_rust, preloaded_context, cached Rust views | Consumes render context, not a persistence projection. Inventory cache/backend callers and retention separately; do not treat permitted rendering as client JSON or session permission. |
...
| Legacy/Rust state backends | state_backend.py and state_backends/{base,memory,redis}.py | Caller and stored-payload inventory still required. Explicit session adapter evidence does not cover actor/render-cache persistence. |
