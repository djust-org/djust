...
The local `feat/declarative-audio-rc5` branch implements the short-cue API,
optional player, both template engines, static asset checks, scoped batches,
activation/mute/volume, bounded resource loading and voices, and Snake's
per-session event cursors. It is not a published release or acceptance of the
full cross-browser qualification above. See `docs/website/guides/audio.md`.

The wire payload includes a server-generated random `scope` tied to one mounted
view and its manifest. This supplies explicit recipient identity for both
existing global transport notifications; bank names and client-provided
selectors are never used for routing. The `_audio_` private-state namespace is
excluded from persistence so scopes and application cursors cannot restore old
cues. Nested child push queues and audio-only ticks are flushed explicitly.

Continuous streaming, speech/TTS, music, synchronized playback and priorities
remain outside this first implementation. The Snake assets are original short
WAV cues generated by its checked-in Python script.
