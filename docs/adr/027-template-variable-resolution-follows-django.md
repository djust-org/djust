...
`:1889` and `render_template_with_dirs` `:1942`); the LiveView path converts through `update_state`
...
**The walk** is `Context::resolve_without_builtins` (`context.rs:881`): value stack first (`get`
...
`lib.rs:3674`) and then injects the sidecar wholesale (`:540-546`; twins `:649-667`, `:868-884`) —
the #2509 exposure.
...
(`context.rs:997`), i.e. `PyObject_GetItem`, which honours `__class_getitem__`, so
...
| `changed_keys` partial re-render (`renderer.rs:1291`, keyed on top-level names from `parser.rs:1256`) | No: change detection is Python-side (`rust_bridge.py:812-908`: container `!=` `:875`/`:902`, immutables, else `id()` `:887`) | models: `_normalize_db_values` (`:87`) turns a Model into a dict so container `!=` detects a field mutation on a stable `id()` — an evidence-backed reason models stay eager |
...
| `Encoded` structural equality (`lib.rs:851`; `Value` itself deliberately has no `PartialEq`, `:841`; template `==` is `renderer.rs:4140`) | No, provided the handle is ignored and facts are compared | — |
...
`Python::attach`, and the walk opens its own (`context.rs:963`). The eager tree buys no GIL-free
...
plain path only, `docs/TEMPLATE_BACKEND.md:180-200`).
...
and the `:3228` decline are deleted. The sink: today's loop body (`context.rs:973-1068`) becomes
...
`context.rs`; `Context::resolve` (`:844`) stays the public entry, and `get_value` (`renderer.rs:3862`)
/ `get_value_safe` (`:3888`) and every tag operand already end there. Django's exact rules, in
...
halves. Every materialisation still routes through `protect_sidecar` (`context.rs:1094`) →
...
(`renderer.rs:3435`) resolves operands through `get_value`, which substitutes `Value::Missing`
...
`evaluate_condition_for_if`'s existing `VariableDoesNotExist → false` arm (`:3428-3430`) as the
...
**Step 2 — dormant-define.** `walk_live` extracted from `context.rs:973-1068` with the SAME
...
`context.rs:205`, and every constructor that starts a fresh frame — `:242`, `:256` — must carry it
as `Clone` does at `:223`). With the flag ON: `FromPyObject`'s fallback produces handle-bearing
...
alias fallback (`context.rs:911-961`; `Context::aliases`' XSS `is_safe` use stays — a different
...
| The walk's terminal re-enters the eager conversion | read `context.rs:1069` | `Ok(current.extract::<Value>().ok())` |
