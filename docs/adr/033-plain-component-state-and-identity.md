...
**D5 — An instance carries a `name`** (*decided: `name`, the HTML and `theme_input` / `toggle_group` word, over Django forms' `prefix`*). `Component(name="service")` makes `event_attrs` add `dj-value-name="service"` to every trigger, so the handler signature convention is `def set_rating(self, value, name=None, **kwargs)` and one handler serves any number of instances — the formset case (`for row in rows: Rating(name=f"row-{row.pk}")`). `event=` stays as what it is, a rename of the verb, documented as such. *Amended (S3, review of #2925):* `name` is an identity only for a class that does not declare a `name` parameter of its own. Twenty-six shipped components (`DatePicker`, `Combobox`, the inputs) mean the HTML field name by `name` and default it non-empty; stamping that on every trigger would have handed an unexpected parameter to every pre-existing handler without `**kwargs`. Those classes keep `name` as the field name and are not named on the wire; a class without a `name` parameter (`Rating`, `Accordion`, …) takes it as identity. Two adjacent compatibility rules landed with it: a handler annotated `value: str` receives the text a typed value stands for (`"4"`, `"true"`), and `event_attrs` renders a non-JSON object (UUID, date, Decimal) as its `str()`, untyped. *Amended (S3):* the earlier sentence making `name` the `dj-if` marker namespace was dropped — that namespace (`_dj_if_id_namespace`, `components/base.py:104`) belongs to the `template_name` render path, which a plain component's inline `template` and `_render_custom` never take; there are no markers to namespace. If a plain component ever renders through that path, `name` is the value to hand it.
