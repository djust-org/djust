...
**Built-in tags — 19 of 25 supported:**
- native Rust (17): `autoescape`, `block`, `comment`, `csrf_token`, `cycle`, `extends`, `firstof`, `for`, `if`, `include`, `load`, `now`, `spaceless`, `templatetag`, `verbatim`, `widthratio`, `with`
- via Python handler (2): `regroup`, `url`

**Built-in tags — unsupported (6):** `debug`, `filter`, `ifchanged`, `lorem`, `querystring`, `resetcycle`
...
- **57.02%** of the Django template tests that reach the engine pass (597 of 1047) <!-- django-suite-claim -->
- Over the whole `template_tests` label the figure is 69.09% (1006 of 1456, 14 skipped). That is not the headline: 409 of those tests never reach any engine (`test_parser`, `test_context`, `test_smartif`, ...) and measure Django against itself, so no engine work can move them.
