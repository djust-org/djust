...
| **Client.js** | ❌ No (smaller pages) | ✅ Yes (~64 KB gz) |
| **WebSocket** | ❌ No | ✅ Yes |
...
- A project's own `{% load %}` tag libraries ARE loaded (#2547, see "Loading a project's template libraries" below); a raw `@register.tag` that consumes a body is bridged when it is a plain wrapper and refused, with the reason, otherwise (ADR-030)
...
- **Raw block tags — bridged when they wrap, refused when they do not (ADR-030):** a raw `@register.tag` whose compile function reads past its own token is probed once at `{% load %}` time. A *wrapper* — one `parser.parse(("end<name>",))` whose node renders its body as-is — is bridged through the same rendered-body path as `simple_block_tag`: the Rust engine renders the body (so `dj-*` attributes, native tags, filters and nested blocks all keep working) and Django's own node wraps it. A tag that parses more than one body segment (an intermediate token such as `{% pane %}`), reads tokens directly (`next_token` / `skip_past`), or keeps only typed children from its nodelist is refused per tag, at parse time, with a `TemplateSyntaxError` naming the reason; the `{% load %}` succeeds and the rest of the library works. A node that renders its body in a modified context (`context.push(...)` or a changed `autoescape` around `nodelist.render`) is refused too: the Rust engine has already rendered the body by the time the node runs, so honouring it is impossible and refusing beats diverging silently. Two limits: the probe compiles and renders the tag once, at `{% load %}` time, with a lenient context (every lookup answers `""`, `context.request` is a bare `HttpRequest`), so a render with side effects fires once; and a compile function that raises without arguments cannot be probed and stays refused, saying so. Give such a tag a native handler (`djust.register_block_tag_handler`) or, on Django 5.2+, port it to `simple_block_tag`.
...
Reference: Django 5.2.17 — `django.template.defaultfilters`, `defaulttags` and `loader_tags` (the engine's `default_builtins`), plus the `i18n`, `l10n`, `tz`, `static`, `cache` libraries.
...
- **98.57%** of the Django template tests that reach the engine pass (1032 of 1047) <!-- django-suite-claim -->
- Over the whole `template_tests` label the figure is 98.97% (1441 of 1456, 14 skipped). That is not the headline: 409 of those tests never reach any engine (`test_parser`, `test_context`, `test_smartif`, ...) and measure Django against itself, so no engine work can move them. (The skip count follows the environment: five of the fourteen are `jinja2` tests, and `jinja2` is not in the lockfile.)

Two result kinds are counted separately because they are different work:

- **ERROR**: the test could not run to an assertion. The 14 remaining errors inspect unsupported Django node, loader-cache, or compiler-metadata APIs. Unknown tags are refused during compilation with Django-style `Invalid block tag` diagnostics. The support-list generator recognizes both these messages and historical unsupported-tag messages when cross-checking saved scoreboards.
- **FAIL**: the test reached an assertion and disagreed with Django. The one remaining failure inspects the exact number of entries in Django's loader cache; its rendering assertions pass. The backend honors `OPTIONS["autoescape"]` and explicit Django `Context.autoescape` overrides; escaping defaults to enabled, and dictionary keys cannot change the policy.

Compilation and runtime errors retain the failing file and token span, including custom-tag exceptions and failures inside included templates or inherited blocks. In debug mode these locations populate Django's `template_debug` metadata; user exceptions retain their original identity.

`include` and `extends` accept compiled template objects, including nested context lookups and Django backend wrappers. Compiled djust templates retain their native AST, so rendering and inheritance reuse the original compilation. `include` also accepts objects implementing `render(context)`. Django node metadata is intentionally unsupported. See [compatibility boundaries](TEMPLATE_COMPATIBILITY_LIMITS.md) for all 15 remaining cases, their reasons, and supported alternatives. These cases remain in the raw score.

**No `template_tests` case segfaults the interpreter any more.** Seven did — a `DjustTemplate` or a type object placed in the context, the #2516 reference-cycle class — and all seven stopped when ADR-027's lazy resolution became the default in #2539. Two mechanisms did it, and neither was aimed at the crashes: nothing walks an object's `__dict__` eagerly, so a reference cycle has no unbounded recursion to overflow, and the segment walk carries Django's metaclass guard, so a `list`-subclass class object never reaches `__class_getitem__`.
...
The number above moves only through `scripts/django-template-suite-baseline.json`, regenerated with `--write-baseline` in the PR that improves it, never edited by hand. That one command now also retargets every `django-suite-claim`-marked line — this page's and the README's, percentage *and* `<ok> of <ran>` counts — so the three places the headline lives move together from one measurement (#2615; `--no-doc-claims` opts out). Tests pin each doc's figure and counts to the file. (The marker is an HTML comment; this paragraph names it without spelling it, so the pin does not read this sentence as a claim.)

The `django-template-suite` CI job runs on every PR **and on every push to `main`**, and is a blocking ratchet inside the `test-summary` AND-gate (#2612/#2522): `compare` exits non-zero when either percentage drops below the committed baseline, or when nothing reached the djust engine.

**On a merge conflict in the baseline, take neither side — re-measure.** Two PRs that both regenerate conflict on this file by construction, and that conflict is the mechanism working: the only correct number is the one measured against the merged tree, so resolve by running the suite again on the merge result. The quiet case is the opposite one: PR A regenerates, PR B does not, and B merges second. Nothing conflicts, and the committed figure was measured against a tree that no longer exists. That is safe in exactly one direction — if B *raised* the number the file under-claims until the next regeneration, and if B *lowered* it the `main` run's blocking `compare` goes red on the merge commit. So the scoreboard can lag reality; it cannot overstate it.
...
Template error: Invalid block tag on line 1: 'badtagname'. Did you forget to register or load this tag?
...
**Error:** `Invalid block tag on line 1: 'tag_name'. Did you forget to register or load this tag?`
