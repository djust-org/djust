...
- **63.42%** of the Django template tests that reach the engine pass (664 of 1047) <!-- django-suite-claim -->
- Over the whole `template_tests` label the figure is 73.70% (1073 of 1456, 14 skipped). That is not the headline: 409 of those tests never reach any engine (`test_parser`, `test_context`, `test_smartif`, ...) and measure Django against itself, so no engine work can move them.
