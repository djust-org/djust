...
- **FAIL**: the test reached an assertion and disagreed with Django. The ten remaining failures cover cycle bindings across scopes (two), non-ASCII debug output (one), a missing render-time error for `block.super` in a base template (one), `ifchanged` state across includes (one), engine/context-wide autoescape policy (three), and inheritance/include behavior (two). Engine-wide autoescape remains explicitly refused by this backend; `{% autoescape off %}` is supported inside templates.
