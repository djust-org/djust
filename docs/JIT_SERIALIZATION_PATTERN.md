...
class PropertyListView(BaseViewWithNavbar):
    ...
    @event_handler()
    @debounce(wait=0.5)  # client collapses the keystroke burst into one send
...
With it off, Models, QuerySets and model lists in the context are serialized
