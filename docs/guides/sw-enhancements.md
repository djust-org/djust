...
djust's client JS (~61 KB gz), CSS, and icons are small but loaded on every page. Django's `ManifestStaticFilesStorage` uses content hashes, making cache invalidation safe but still requiring a network check.
