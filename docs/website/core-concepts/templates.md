...
never makes other input safe — ``{{ user_text|shout }}`` is escaped
whether or not ``shout`` is ``is_safe=True``. A filter that *produces*
markup must mark its own output, with ``mark_safe`` or ``format_html``;
the flag does nothing for it:
