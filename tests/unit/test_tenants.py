...
class TestSubdomainResolver:
    ...
    def test_excludes_www(self):
        ...
        with patch.object(resolver, 'get_config', side_effect=lambda key, default=None: 
...
class TestPathResolver:
    ...
    def test_resolve_from_path(self):
        ...
        with patch.object(resolver, 'get_config', side_effect=lambda key, default=None: 
                          1 if key == 'TENANT_PATH_POSITION' else 
                          ['admin', 'api', 'static', 'media'] if key == 'TENANT_PATH_EXCLUDE' else default):
            tenant = resolver.resolve(request)
    ...
    def test_resolve_from_path_position_2(self):
        ...
        with patch.object(resolver, 'get_config', side_effect=lambda key, default=None: 
                          2 if key == 'TENANT_PATH_POSITION' else 
                          ['admin', 'api'] if key == 'TENANT_PATH_EXCLUDE' else default):
            tenant = resolver.resolve(request)
    ...
    def test_invalid_tenant_id_format(self):
        ...
        with patch.object(resolver, 'get_config', side_effect=lambda key, default=None: 
                          1 if key == 'TENANT_PATH_POSITION' else [] if key == 'TENANT_PATH_EXCLUDE' else default):
            tenant = resolver.resolve(request)
...
class TestResolveTenant:
    ...
    def test_applies_default_tenant(self):
        ...
        from djust.tenants.resolvers import resolve_tenant, TenantInfo
