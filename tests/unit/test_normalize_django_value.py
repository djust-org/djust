...
class TestCallable:
    """callable -> None, on the one channel that still drops it (#2621).
    ...
    Since #2621 a callable crosses RAW and the resolution sink applies
    Django's own call rules (``alters_data``, ``do_not_call_in_templates``, a
    propagating raise). The ``template_resolve_lazy`` escape hatch that once
    kept the drop was deleted in ADR-027 Step 5 (#2628).

    One channel keeps the drop, and it is what these cases assert:
    ``state_roundtrip=True`` — the session / signed-snapshot boundary, written
    by an encoder-less serializer that cannot hold a live object.

    The live behaviour is pinned in
