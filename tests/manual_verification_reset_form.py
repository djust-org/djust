...
from demo_app.views.forms_demo import ProfileFormView  # noqa: E402
        ...
        if not state_consistent:
            ...
            for key in mount_state:
                if mount_state.get(key) != reset_state.get(key):
                    print(
                        f"     {key}: mount={mount_state.get(key)!r}, reset={reset_state.get(key)!r}"
                    )
        else:
            print("   ✅ PASSED: State consistent")
    ...
    if all_consistent:
        ...
        for r in results:
            if not (r["state_consistent"] and r["values_match"]):
                print(
                    f"   Cycle {r['cycle']}: state_consistent={r['state_consistent']}, values_match={r['values_match']}"
                )
        return False
