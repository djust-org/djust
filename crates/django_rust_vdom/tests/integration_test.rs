...
use django_rust_vdom::{diff, parse_html};
...
fn test_form_validation_errors_with_real_html() {
    ...
    let remove_patches: Vec<_> = patches
        .iter()
        .filter(|p| matches!(p, django_rust_vdom::Patch::RemoveChild { .. }))
        .collect();
    assert_eq!(
        remove_patches.len(),
        2,
    ...
    let attr_patches: Vec<_> = patches
        .iter()
        .filter(|p| matches!(p, django_rust_vdom::Patch::SetAttr { .. }))
        .collect();
    assert!(
        attr_patches.len() >= 2,
...
fn test_conditional_div_with_whitespace() {
    ...
    assert!(
        patches.iter().any(|p| matches!(
            p,
            django_rust_vdom::Patch::SetAttr { key, value, .. }
...
fn test_deeply_nested_form_structure() {
    ...
    let remove_patches: Vec<_> = patches
        .iter()
        .filter(|p| matches!(p, django_rust_vdom::Patch::RemoveChild { path, .. } if path.len() > 5))
        .collect();
...
fn test_patch_indices_account_for_whitespace() {
    ...
    assert!(
        patches.iter().any(|p| matches!(
            p,
            django_rust_vdom::Patch::SetText { .. }
        )),
