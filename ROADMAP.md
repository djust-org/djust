...
### v1.2.1-14 — Template and rendering follow-ups (drain bucket → ships in 1.2.1)

*Kind:* rendering correctness, Python/Rust parity, DX. Filed during the v1.2.1 drain or by users since; no triage file.

| Priority | Task | Milestone |
|---|---|---|
| ~~**P1**~~ | ~~#3018 — Handler-metadata script is injected before every `</body>` string (use the #3017 masked `</body>` lookup)~~ ✅ (#3053) | v1.2.1 |
| ~~**P1**~~ | ~~#3019 — Raw-text masker is quadratic on many `<script` without `>`~~ ✅ (#3053) | v1.2.1 |
| ~~**P1**~~ | ~~#3020 — T018 reports quoted literals and filter names in `{% if %}` comparisons as undefined~~ ✅ (#3053) | v1.2.1 |
| ~~**P1**~~ | ~~#3024 — Third-party inclusion tags crash in LiveView templates ("context must be a dict")~~ ✅ (#3053) | v1.2.1 |
| ~~**P1**~~ | ~~#3025 — `{% badge %}` output is unstyled (1.2.1: document as unstyled; shipping CSS and the BEM class audit → 1.3)~~ ✅ docs (#3053); CSS → 1.3 | v1.2.1 |
| ~~**P1**~~ | ~~#3026 — `highlight_code` drops spaces between bare words (bash)~~ ✅ (#3053) | v1.2.1 |
| ~~**P1**~~ | ~~#3030 — Python and Rust root locators disagree on a quoted value containing `<… dj-root>`~~ ✅ (#3053) | v1.2.1 |
...
| ~~**P1**~~ | ~~#3043 — `live_form` / `live_field` / `live_errors` render escaped HTML (tag markup safe, user values escaped)~~ ✅ (#3053) | v1.2.1 |
| ~~**P1**~~ | ~~#3044 — `live_input`, `djust_skeleton`, `djust_track_static` have no Rust handler~~ ✅ (#3053) | v1.2.1 |
| ~~**P1**~~ | ~~#3045 — VDOM `write_html` escapes text inside noscript, xmp and other raw-text elements~~ ✅ (#3053) | v1.2.1 |
...
| ~~**P1**~~ | ~~#3051 — Offline indicator (`show_when="offline"`) and offline banner never show; indicator text and class never switch~~ ✅ (#3057) | v1.2.1 |
...
| #3031 | dj-root vs dj-view root precedence: one rule for Python and the Rust VDOM that skips embedded/sticky child roots (either current rule moves some pages' root) |
...
>   the #2510 panic, fixed in PR #2514 in the same extraction walk), two security
...
> - **v1.2.0-5 — the discussion #2437 cluster**: the documented `dj-view`
>   context variable does not exist, so the README quickstart cannot mount;
>   plus the component-escaping half deferred out of #2501.
...
|---|---|---|---|
| ~~**P1**~~ | ~~Run Django's own template test suite against `DjustTemplateBackend` (#2517)~~ ✅ **PR #2525** | A script that checks out the Django tag matching the installed version, patches `tests/template_tests` so its engine fixtures route through the djust backend, runs it, and prints `OK / FAIL / ERROR` plus a percentage. Django's ~1,600 template tests are a corpus maintained by Django, covering tags, filters, loaders, context, autoescape and inheritance — every axis our own differential has had to grow one blind spot at a time (#2345). Ship as a **non-gating** CI job first (the #1534 rule: a job the dev machine cannot fully mirror stays `continue-on-error` until it is runner-green), then promote it to a **ratchet**: the percentage on a PR may not drop below `main`'s. The baseline number is the arc's scoreboard and goes in the README in v1.2.0-4. **Shipped 2026-09-02.** Seam: an in-process `Engine` subclass overriding only `from_string`/`get_template`, installed after `django.template.backends.django` binds the real class — nothing in Django's checkout is edited, so it re-applies to any tag. Baseline in `scripts/django-template-suite-baseline.json`: **43.55%** on the 1047 tests that reach the engine (456 OK / 227 FAIL / 364 ERROR), 59.41% across the whole label; the headline is the engine subset because 409 tests never construct an engine. Seven `template_tests` cases segfault the interpreter (the #2516 class) and are isolated per test. The ubuntu runner reproduced the macOS baseline to the digit on the job's first run; promotion to a ratchet is #2522. The first run filed #2518, #2520, #2521 and the `<!--dj-if-->` leak in #2519; the process cost (four pre-push hook runs on one commit) went to #2526. | v1.2.0 |
| ~~**P1**~~ | ~~Feature-gate the engine/LiveView boundary (#2519)~~ ✅ **PR #2534** | A `liveview` cargo feature on `djust_templates` (default on) gating the `djust_vdom::VNode` cache in `loop_cache.rs`, the `dj-if` boundary markers, and the `djust_components` path in `renderer.rs`; plus a CI step `cargo check -p djust_templates --no-default-features`. This is not the split — it is the measurement of what a split would have to carry, and a tripwire against LiveView concerns leaking into the engine. Prerequisite for the decision gate. **Shipped 2026-09-02.** Root cause of the leak: parallel-path drift between the two plain Rust entries (`render_template` stripped markers post-render; `render_template_with_dirs`, the backend's entry, never did). Fixed structurally: an `emit_dj_if_markers` switch on `Context`, plain entries off, strip deleted (it cost 26–30% of render time), `{% include … only %}` copies it. `liveview` feature gates the VNode loop cache, the components path and marker emission; registries/parser/sidecar stay by decision; CI `--no-default-features` check + `cargo tree` pin. Scoreboard 43.55% → 44.03% (five of six `dj-if` cases; the sixth is #2528). Behaviour change: `LiveComponent.template_name` HTML no longer carries leaked markers (#2530). Findings: #2528 #2529 #2530 #2531 #2537. First push to survive the full hook via the repo-local SSH keepalive (#2526). | v1.2.0 |
| ~~**P2**~~ | ~~#2532 (carried from v1.1.1-2)~~ ✅ **PR #2543** | **Model-backed render benchmark** (`PERFORMANCE_BRAINSTORM.md` §7). Still no `models.Model` under `tests/benchmarks/`, so the Python↔Rust boundary cost is invisible to CI. v1.2.0-2 changes exactly that boundary, so this benchmark is its performance gate and must land first. Fixture requirements unchanged: a `@property` column, a reverse-relation call, an un-`select_related` FK, an attribute-change event, and instrumentation of whether the `SetText` fast path was taken. **Shipped 2026-09-02 (PR #2543)**: lists never cross the sidecar — the presenter shape is the sidecar path (302 crossings / 50 queries per render); the four `list_*` variants' zero-crossing count is the ADR-027 flip's invariant. | v1.2.0 |
| ~~**P3**~~ | ~~#2533~~ ✅ **PR #2546** | **Generate the backend's supported/unsupported lists from the engine.** `docs/TEMPLATE_BACKEND.md:281-283` says `escapejs` is unsupported and offers a `json.dumps` workaround; `{{ s\|escapejs }}` renders Django's exact output today (verified 2026-09-01), and `:454` still lists it as a todo. Hand-maintained lists drift; emit them from the filter and tag registries the way `client-sizes.json` is emitted (#2138), and check them the same way. **Shipped 2026-09-02 (PR #2546)**: `scripts/generate-template-backend-lists.py` emits both lists from the registries (57/57 filters, 18/25 tags), cross-checks them against the Django-suite results, and runs in pre-commit and CI; the drift it found in the hand-written lists is #2540. | v1.2.0 |
...
| Priority | Issue | Summary | Target |
|---|---|---|---|
| ~~**P1**~~ | ~~ADR-027 — variable resolution follows Django's lookup rules at one sink (#2535)~~ ✅ **PR #2538** | **ADR-027 — variable resolution follows Django's lookup rules at one sink.** Django resolves `{{ a.b.c }}` one segment at a time: dict key, then attribute, then list index; a callable is called unless it sets `do_not_call_in_templates`, and one that sets `alters_data` renders as `string_if_invalid`; a lookup that raises is silent only if the exception sets `silent_variable_failure`. djust today converts the context to a `Value` tree up front (`djust_core::Value`, with `Value::Encoded` for anything without a variant) and reaches attributes afterwards by enumerating container shapes in the sidecar (#2509). The ADR decides three things: **(a)** an opaque value becomes a live-object handle resolved lazily at lookup time by exactly those rules, at ONE sink; **(b)** what stays eager — models under the `_ALWAYS_EXCLUDED_FIELDS` floor (`serialization.py:57`), and whatever the msgpack snapshot round trip (the #2484 class) and `changed_keys` partial re-render need materialised; **(c)** the security posture — the floor must hold at the lazy sink too, or models keep the eager path. GIL cost is not a new class, because the render already holds the GIL. Options considered per #1056; every cited position grep-verified per #1197. **Shipped 2026-09-02 (e7d74374).** Option A's resolution rule with Option C's storage split; 34-shape probe table; a second segfault mechanism bisected onto #2516; the #2532 spike's premise correction absorbed (model lists never cross the sidecar, so the handle is for nested opaque objects only); ADR-024 erratum. Honest flip expectation ~7 of 1047 cells + 1 of 7 crashes, the rest being v1.2.0-3 work. Implementation is #2539 (four movements). | v1.2.0 |
| ~~**P1**~~ | ~~#2539~~ ✅ **PR #2553 / #2569 / #2620** | **Implement ADR-027 as dormant-define → wire → flip.** (1) Characterization tests against the CURRENT sidecar for every open instance — #2502 (`do_not_call_in_templates` renders the marker dict), #2504 (a filtered or dict-view `{% for %}` operand cannot reach attributes), #2505 (a loop variable shadowing an outer name resolves against the outer object), #2513 (the page-shell path has no sidecar at all), #2516 (a reference cycle in the context segfaults the interpreter; the #2517 runner isolates seven `template_tests` cases of it), #2542 (a plain object inside a top-level list/tuple cannot reach attributes on the LiveView path) — each with a gate-off sibling (#1468); the closed security findings #2506 (any exception swallowed, fail-open) and #2507 (`{{ c.unmount }}` runs a mutator during render) become permanent pins. (2) The lazy sink, defined and tested but not routed. (3) The flip, gated by the Django-suite percentage and the v1.2.0-1 benchmark. (4) Delete the enumeration arms. Expected dividend (the #1898 rule): the flip will surface divergences the sidecar hid — budget for them; they are findings, not regressions. **Movement 1 shipped 2026-09-02 (PR #2553)**: 402 characterization cases over 45 shapes on every render path, the #2506/#2507 pins made permanent, the #2516 crashes held in subprocess cells, and `Context::walk_live` defined with eleven Rust tests but routed nowhere (scoreboard unchanged at 44.03%, the no-behaviour-change proof). Movement 2 requirements are recorded on #2539. **Movement 2 shipped 2026-09-02**: the sink is routed at one call site behind `LIVEVIEW_CONFIG['template_resolve_lazy']` (default off; bytes identical to before); flag on, 29 of 37 wrong cells answer Django's bytes and the #2516 crash cells stop crashing; six cells held for movement 3 (component/reader rows + four LiveView rows where `normalize_django_value` drops callables). **Movement 3 shipped 2026-09-03**: the flag defaults ON (both the Python default and the Rust thread-local, which a fresh thread reads); `False` is the escape hatch, deleted with the enumeration arms in 1.3.0. Scoreboard 70.39% → 70.96% engine (737 → 743 of 1047), 78.71% → 79.12% whole (the +6/-7-crash delta measured identically against three different bases as the week's other template PRs landed underneath it), **0 OK→FAIL transitions**, and all seven segfaulting cells stop crashing (the ADR predicted one). #2502/#2504/#2505/#2510 close outright on both paths against Django's bytes. Six cells stay held with named reasons (the transient `safe` bit and the gated callable arm are movement 3's follow-on PR). **Closed 2026-09-03** at the 1.2.0 boundary — movements 1–3 shipped; the six held cells (#2621) and movement 4 (delete, #2628) carry forward to 1.3.0 by design. | v1.2.0 |
| ~~**P2**~~ | ~~#2509~~ ✅ closed with a pointer | **Floor the tag-handler sidecar at the sink, not by enumerating container types.** Absorbed by the two rows above — the "enumerate container types" mechanism is what ADR-027 replaces. Stays open until the flip lands, then closes with a pointer rather than by adding one more type. **Closed 2026-09-03**, pointing at ADR-027 movement 3 (PR #2620) as promised. | v1.2.0 |
...
drops the parent block on the backend path); they belong here. The #2539 planning and
review added #2549 (`from_string` compiles eagerly, so 111 suite cells error at
construction), #2550 (`Context.use_l10n` / `autoescape` dropped at flatten), #2551 (a
render-path panic) and #2552 (the parser panics on any non-ASCII byte inside `{{ }}`).
Rows run as conformance-loop pipelines (`.pipeline-templates/conformance-state.json`):
one family, one loop, one review.

| Priority | Issue | Summary | Target |
|---|---|---|---|
| ~~**P1**~~ | ~~#2549~~ ✅ (see the shipped row below) | **111 `TemplateSyntaxError`-timing cells.** `DjustTemplate` never compiles at `from_string` / `get_template`, so a `TemplateSyntaxError` Django raises at construction surfaces at render, and every `assertRaises` around construction errors instead. The #2557 plan found it is the timing half of the error-diagnostics row, PR-B; the smallest change per cell in the bucket (compile eagerly, raise at the same point Django does). Largest single family on the scoreboard.  | v1.2.0 |
| ~~**P2**~~ | ~~#2556~~ ✅ **PR #2595 / #2596** | **Six built-in tags the parser does not know:** `autoescape`, `filter`, `lorem`, `debug`, `resetcycle`, `querystring` (by grep of the tag match in `parser.rs` / `tags.rs`, 2026-09-01). `autoescape` first — it is what makes the `needs_autoescape` flag observable, which #2284 left as "a decision, not a patch". `querystring` is Django 5.1+; gate on version. **Closed 2026-09-03.** All six on the parser/registry: `autoescape`/`filter`/`resetcycle` native Rust, `lorem`/`debug`/`querystring` Python handlers. Scoreboard 44.03% → 62.85% across the pair. | v1.2.0 |
| ~~**P1**~~ | ~~#2547~~ ✅ **PR #2594** | **`{% load app_tags %}` imports the Django template library.** Today `parser.rs:812` keeps the library names so inheritance can re-emit the tag, and nothing imports them; a project's existing `templatetags/` modules are invisible to the Rust engine and must be re-registered by hand through `register_tag_handler` / `register_django_filter` (`template_filters.py:113`). Resolve the name with Django's own `get_installed_libraries()` and `django.template.library.import_library`, then bridge each entry by kind: `filters` through `register_django_filter`, `simple_tag` / `simple_block_tag` / `inclusion_tag` through the tag-handler sidecar (ADR-001). The `builtins` option of `TEMPLATES` gets the same treatment. Django's `template_tests/test_custom.py` is entirely this. **Closed 2026-09-03.** Django's own `get_installed_libraries()` / `import_library`; filters through `register_django_filter`, simple/block/inclusion tags through the tag-handler sidecar. | v1.2.0 |
| **P2** | #2558 (carried from v1.1.1-2) | **`{% translate %}` / `{% blocktranslate %}` and the `_("…")` literal.** Still `Unsupported template tag`, so no i18n project can use the Rust engine at all. Sequence per #1077: bridge through the Python tag registry first (gettext runs in Python, the catalogs stay Django's), measure, port catalog lookup into Rust only if the numbers say so. Pieces 2 and 3 of #2221 (localized month/day names, per-locale `DATE_FORMAT`) are catalog lookups too and ride the same bridge; `{% localize %}` and the `tz` library tags come with them.  **Shipped 2026-09-03 (PR #2597).** `{% load i18n %}` / `l10n` / `tz` bridges Django's own library, so the catalogs, plural rules, `%`-formatting and escaping are Django's — the bridge runs Django's compile functions and nodes. Three mechanisms: a fourth tag-registration kind whose body crosses as un-rendered SOURCE (`{% blocktranslate %}`'s body IS the msgid, so Django's lexer must build the `%(var)s` placeholders and its parser must be the one to refuse a nested `{% block %}`), a render-time translator hook at the literal recogniser for `_("…")`, and native Rust scope nodes for `{% language %}` / `{% localize %}` / `{% localtime %}` / `{% timezone %}` that switch Python's thread-local and re-push the locale/zone state around their children. Scoreboard **62.85% → 70.39%** (658 → 737 of 1047, +79 cells and none worse; whole label 73.28% → 78.71%), measured on this branch merged with `main` against `main`'s own committed baseline: all 116 cells of `syntax_tests/i18n` pass and the `blocktrans` (27) / `trans` (15) error classes are gone (FAIL 212 → 183, ERROR 177 → 127). What the issue got wrong: `{% blocktranslate %}` cannot go through any existing registry (the body is data, not a nodelist); nothing about translation "rides `render_env`" — gettext runs on the render thread and `render_env` matters only for the INVERSE direction; and `{% localize %}` / the `tz` tags have zero scoreboard cells. Residues, each a named test: the three `tz` filters and the `{% localize off %}` DATE half (#2216 / #2221-3), and `{% localtime on %}` under `USE_TZ = False`. | v1.2.0 |
| ~~**P2**~~ | ~~#2557~~ ✅ **PR #2623** | **Template errors that Django's debug page can render.** The backend raises a generic `Error rendering template: …` (`docs/TEMPLATE_BACKEND.md:426`) and attaches no `template_debug` to it (`rendering.py`, none), so the technical 500 shows no template name, line, or source excerpt. Attach Django's `template_debug` dict (`name`, `line`, `during`, `source_lines`, `top`, `bottom`) from the parser's position information, and raise `TemplateSyntaxError` at parse time the way Django does. An unknown filter already refuses at parse time (#2419); extend that contract to every error. **Shipped 2026-09-03.** `template_debug` (`name`, `line`, `during`, `source_lines`, `top`, `bottom`) attached, verified against Django's own `Template.get_exception_info` and a real `technical_500_response` render; also closed the #2552 non-ASCII parse panic. Engine 70.96% → 71.35% (+4, 0 regressions). | v1.2.0 |
| ~~**P2**~~ | ~~#2549~~ ✅ **PR #2593, closed 2026-09-03** | **`DjustTemplate` parses at construction, so `TemplateSyntaxError` surfaces where Django's does.** The Rust parse ran at first `render`, so a syntax error surfaced one call later than `Engine.from_string` / `get_template`, and a defect in an untaken branch never surfaced at all; 111 suite cells asserted the timing. Parse in `DjustTemplate.__init__` through `TEMPLATE_CACHE`, raise `DjustTemplateSyntaxError` (Django's `TemplateSyntaxError` + `RuntimeError`), and promote the two render-time refusals Django has at parse (`Unsupported template tag`, `Unknown templatetag argument`). **Shipped 2026-09-02**: parse at construction; 47.09% engine (+32 cells); the 75 remaining `not raised` cells are engine-grammar gaps filed as follow-ups (#2576–#2580; message text #2581). Final residue count at close: **65** `TemplateSyntaxError`-not-raised cells (from 111), plus 5 of a different class. Mechanism done; residue sliced into #2576–#2581 and #2617. | v1.2.0 |
| ~~**P3**~~ | ~~#2563~~ ✅ **PR #2607** | **`{% url %}` fails soft to `''` where Django raises `NoReverseMatch`.** Inherited, not decided. With the Django suite in place this becomes a measurable choice: `template_tests/syntax_tests/test_url.py` encodes Django's answer. Decide, document, and either match it or write the rule down. **Closed 2026-09-03.** Matches Django: raises `NoReverseMatch`. | v1.2.0 |
| ~~**P1**~~ | ~~#2548 (security)~~ ✅ **PR #2554** | **Custom filters declared `is_safe=True` must not mark UNSAFE input safe.** A registered filter with `is_safe=True` marks its output safe unconditionally; Django's contract is `is_safe` *preserves* safety, so an unsafe input stays unsafe (the same class as the #2285 advisory). `{% load %}` (next row) widens the filter surface to every library a project installs, so this lands FIRST. Fix at the single sink in `renderer.rs`; re-point the test that pins the wrong behaviour; probe encoded variants end-to-end (#1825). | v1.2.0 |
...
| Priority | Issue | Summary | Target |
|---|---|---|---|
| ~~**P1**~~ | ~~#2559~~ ✅ **PR #2573** | **Importing the backend must not import the LiveView stack.** `djust/__init__.py:10` imports `live_view` eagerly, so `djust.template_backend` pulls `channels`, `presence` and over 160 other modules into a project that only wanted templates. Make the package init lazy (PEP 562 `__getattr__` for the public names) and pin it: a test that imports the backend in a fresh interpreter and asserts `channels` is absent from `sys.modules`. Non-breaking. **Closed.** | v1.2.0 |
| ~~**P2**~~ | ~~#2560~~ ✅ decided | **Decide the extras shape for a templates-only install.** `channels[daphne]` and `msgpack` are hard dependencies (`pyproject.toml`). Moving them behind `djust[live]` makes a bare `pip install djust` lose the WebSocket stack — a breaking change that needs a deprecation cycle and belongs at a 2.0 boundary — or there is an additive alternative (a documented `djust[templates]` that adds nothing, or a metadata-only companion). A decision row: write the options down, pick one, record it in ADR-007's taxonomy table. **Decided 2026-09-02**: no extra in 1.x (an extra can only add deps); ship the additive half (#2559 lazy init + C016); `djust[live]` at the 2.0 boundary with a 1.x deprecation warning. Written in `docs/TEMPLATE_BACKEND.md` Quick Start. | v1.2.0 |
| ~~**P2**~~ | ~~#2561~~ ✅ **PR #2629** | **"Rust templates for any Django view" as a headline, with numbers.** The README and djust.org get a section saying what `docs/TEMPLATE_BACKEND.md` already knows: any `TemplateView` or `render()` call, one `TEMPLATES` entry, no client.js. It carries two numbers and nothing softer — the Django-suite percentage from v1.2.0-1, and the measured "7–11x on filter-heavy templates, no faster on static markup" the README already claims (`README.md:22`). The scaffold already configures the backend; say so. **Shipped 2026-09-03.** README section carrying 71.35% (pinned by the generalized `<!-- django-suite-claim -->` test) and the existing 7–11x benchmark figure. djust.org copy not yet done — the issue's own site half is a separate repo and stays open there if wanted. | v1.2.0 |
| ~~**P3**~~ | ~~#2562~~ ✅ **PR #2574** | **A `C`-category system check for the `TEMPLATES` shape.** Warn when `DjustTemplateBackend` is configured without a `DjangoTemplates` entry after it (admin and contrib templates need one; the scaffold emits both, `scaffolding/templates.py:201`), and when two entries share a `NAME`. Next free `C5xx` id, checked against the registry at registration time (the #1561 V012/V014 collision). **Closed.** | v1.2.0 |

### v1.2.0-5 — the discussion #2437 cluster: getting-started is broken (drain bucket → ships in 1.2.0)

*Goal:* a reader who follows `README.md` gets a page that mounts. Opened
2026-09-03 from [discussion #2437](https://github.com/djust-org/djust/discussions/2437),
an external report whose second half we closed as user error on #2501 and which
was in fact our own quickstart. Four issues, one of them release-blocking.

| Priority | Issue | Summary | Target |
|---|---|---|---|
| **P0** | #2631 | **Every documented `dj-view="{{ dj_view_id }}"` renders an EMPTY attribute.** `dj_view_id` has never existed in the code — `git log -S` finds no commit; it lives in 18 Markdown lines and nowhere else. Django resolves it to `''`, so the snippet emits `dj-view=""`, `03-websocket.js:1324` refuses the mount and `:1340` warns. Verified end-to-end on 1.2.0rc1 through a real HTTP GET. Two further phantoms: `{{ view_name }}` (`schema.py:1302,1348`, the MCP surface, so agents generate it too) and `{{ view_id }}` (`docs/guides/turbonav-integration.md:69`). `docs/website/api-reference/liveview.md:73` asserts `super().get_context_data()` provides it; `QUICKSTART.md:180` offers the broken form as the *fix* for the sibling warning. Docs split 45 correct / 18 broken; every shipped scaffold emits the dotted path. **Blocks 1.2.0 GA** — it breaks the front-page quickstart for every new user. | v1.2.0 |
| **P2** | #2632 | **`autoMount` picks a valueless `dj-view` on a sticky/embedded root.** `03-websocket.js:1316` uses a bare `[dj-view]` while its two siblings at `:542` and `:688` use `[dj-view]:not([dj-sticky-root])`; `{% live_render %}` emits a valueless `dj-view` (`live_tags.py:1434,1900,2207`), so document order decides whether the page mounts. Same warning as #2631 but reachable with a correct template, so the docs fix does not close it. #1646. The `:1319` fallback is also dead code (`[dj-view]` strictly contains `[dj-root][dj-view]`). | v1.2.0 |
| **P2** | #2633 | **Component markup is still ESCAPED on all four render paths** — "PR 2 of #2501", deferred in PR #2508's body and never filed, so the reporter's original complaint still reproduces. The suite carries a `strict=True` xfail citing an issue number that did not exist (`test_object_attribute_resolution_2501.py:153`). Needs *two* mechanisms, not one: reorder `_collect_safe_keys` past `normalize_django_value` on the LiveView path (`rust_bridge.py:988-1002` — a Component is flattened to a `SafeString` *after* the scan, so it never becomes an `Encoded` and #2621 cell O cannot reach it), plus the value-carried safe bit for the plain path and for `{{ c.render }}`, which no name-keyed scan can ever cover. Decided: `{{ component }}` renders unescaped rather than requiring `|safe` — Django's `conditional_escape`/`__html__` contract, and `|safe` at the call site cannot catch anything the component already built. Open question: #2621 pins *escaped after a msgpack round trip* as intended, which for a component means non-deterministic output; reachability unestablished. **Scope now also covers the unescaped `_render_custom` fallback tier** (`base.py:349` blanket-`mark_safe`s six raw f-string sites in `components/ui/`); bounded — the Rust and template tiers escape, so it needs `_RUST_AVAILABLE == False`. | v1.2.0 |
| **P3** | #2634 | **Teach `{{ component }}` as the canonical spelling; sweep 27 `{{ x.render }}` occurrences across 15 files.** The #2511 follow-up that was recorded as "filed separately" and never filed. Byte-identical output (`__str__` is `return self.render()`), but `.render` depends on auto-call: under `LIVEVIEW_CONFIG['template_auto_call'] = False` it prints `<bound method …>` into the page while `{{ component }}` still renders. Sequence before #2633 so the `|safe` caveat stays accurate mid-flight. Also decides whether orphaned `docs/website/guides/components.md` gets linked or removed. | v1.2.0 |

### v1.2.0-6 — transport fidelity, wire versioning, and check coverage (drain bucket → ships in 1.2.0) ✅

*Goal:* close the defects filed by an external source review of `main @ 704ab688`
(2026-09-15), each verified by the reviewer against the tree before filing. Two
are **recurrences** of closed issues whose fixes covered only one of several
parallel paths — the #1646 class this repo names as its most-recurring failure
mode — and two were accompanied by a test that passes while the bug executes,
which is why they survived. #2821 and #2823 landed separately as PR #2835: both
had been left as uncommitted worktree state and were rescued rather than
re-derived.

| Priority | Issue | Summary | Target |
|---|---|---|---|
| **P1** | #2829 | **Server-initiated patches (tick/async) still desync the VDOM version.** The `Expected v7, got v8` class #1677 closed for `push_to_view` self-broadcast is live on the two paths that fix did not touch: the client buffers a server-initiated frame (`03-websocket.js:865-874` `break`s before `handleServerResponse`, so its version is never applied) while the server has already allocated that version (`_next_version_armed` runs for `_tick_once` and `_run_async_work`), so the next applied frame fails the strict `clientVdomVersion !== data.version - 1` check (`02-response-handler.js:58`) and forces a full `request_html` recovery. `tests/js/event_sequencing.test.js:166` asserts only that the buffer drained — the flush it asserts is what fires the mismatch. #1646. | v1.2.0 |
| **P1** | #2830 | **`_run_async_work` renders without `_render_lock`** — the one render path of four that does. `server_push`, `db_notify` and `_tick_once` all acquire it; the async path's body contains no reference at all, while it calls `render_with_diff` and `_next_version_armed`. `websocket.py:1556` documents that the render helper's *caller must already hold the lock*, so this violates a stated precondition, on a PyO3 view whose VDOM state is not thread-safe. It is also the enabler for #2829 — a lock-free async render is what can put a server-initiated frame in flight mid-event. | v1.2.0 |
| **P2** | #2831 | **Dotted `dj-keydown.enter` / `.escape` / `.space` never fire.** The delegated selector uses `closest('[dj-keydown]')` and the handler reads `getAttribute('dj-keydown')`, so the dotted form matches neither — and because `keyHandler` is then dot-free whenever it is truthy, `requiredKey` is always null: the modifier-parsing block is dead by construction, not merely unreached. `tests/js/dj-input-modifier-warning-1999.test.js:89` writes the broken form and asserts only that no warning was emitted, so it passes *because* the directive is ignored. Secondary: even once wired, only enter/escape/space are mapped, so `.arrowup` would match any key. | v1.2.0 |
| **P2** | #2832 | **`dj-shortcut` / `dj-click-away` keep firing after the attribute is removed.** #2108 fixed exactly this for the `dj-window-*`/`dj-document-*` registry — its comment names the failure mode verbatim ("morphdom PATCHES a surviving element's attributes … would otherwise keep dispatching a directive the template no longer declares") — but `_sweepOrphanedScopedListeners` still evicts on `!document.contains` alone. With `_isHandlerBound` keying a WeakMap by element, a surviving element keeps both its bound-marker and its listener. #1646. | v1.2.0 |
| **P2** | #2825 | **V008's documented remedy cannot work.** Its return-annotation escape hatch walks `tree.body` only (methods are never collected) and compares a bare name set against the dotted `_get_call_name`, so a call through a module never matches either: `game.claim` and `game.helper` both miss; only a bare `helper()` hits. Recurrence of closed #1609/#1628. `# noqa: V008` still works, which is why it is P2 not P1. | v1.2.0 |
| **P2** | #2827 | **T018 false-positives on framework mixins that set `self.x` dynamically** (`FormMixin.form_setup` → `form_data`/`field_errors`), because the extraction skips any class whose module starts with `djust.`. Found dogfooding the check against `demo_project`; the module involved is unrouted, but the same false positive would hit any routed app using `djust.forms.FormMixin`. Filed by the check's own author, not the external review. | v1.2.0 |
| **P2** | #2833 | **T018 does not fire on `{% extends %}` templates in the system-check path, while `djust_typecheck` does.** The skip itself is a documented, deliberate v1 trade-off — this is about the divergence and its invisibility: on a two-LiveView app whose templates both extend, `manage.py check` and `djust_check` print "all checks passed" having examined zero templates, while `djust_typecheck` reports the same variable on the same tree. The docs claim the two entry points "can never drift apart". | v1.2.0 |
| **P3** | #2834 | **The tick path resolves `_skip_render` + `_force_full_html` differently from the three paths it cites as its model** (tick renders, `dispatch_event`/`server_push`/`db_notify` skip). The new behaviour is arguably the correct one — the commit's own #1646 reasoning says so — which makes this a propagation-or-document task, not a revert. Follow-up to #2822 / PR #2826. | v1.2.0 |
...
localization — the latter carrying two folded-in consequences that measurement
showed are a *different* root cause, now split out as #2250. **#2238** is a structural pin that counts prose — a docstring
...
| Priority | Issue | Summary | Target |
|---|---|---|---|
| ~~**P1**~~ | ~~The Python Decimal converters are still lossy (#2239)~~ ✅ | ~~`normalize_django_value` / `__default__` return `float()`~~ — Fixed: the audit found the pair is one function with **three** destinations, so each takes the representation its destination needs. Template context keeps the `Decimal` (Rust renders it identically to Django); the wire takes the exact digit string (byte-for-byte Django's own encoder); a round trip back onto the view keeps `float` via one new chokepoint, `decimal_for_state_roundtrip`. Two of the three cited blockers were corrected by running them: the parity invariant keeps holding once restated as `dumps(normalize(x)) == dumps(x)` (stronger, and still covers `Decimal`), and `runtime.py:2561` was never at risk — its state is JSON-round-tripped first; the real unguarded consumers are the ten Django session writes. Residue tracked in #2252 (tagged lossless round trip) and #2253 (`floatformat`/`add` parse a `Value::Decimal` through f64). | v1.1.1 |
...
| **P2** | A scientific-form Decimal's coefficient is not localized (#2242) | Past Django's `>200`-digit cutoff a Decimal renders scientific, and `localize_number_with` bails on any string holding an `e`: `1.230E-250` stays `1.230e-250` where Django gives `1,230e-250`. Not a regression — the previous release rendered an f64 further from Django than either. The issue folds in `truncatechars` and `make_list\|first` as "the same root cause", but measuring after the fix falsified that: those filters never reach `localize_number` (Django's are `@stringfilter` and consume `str(Decimal)`; djust's consume the numberformat rendering), and their divergence is not confined to the cutoff — `Decimal('1E-9')` diverges at nine digits. Split out as **#2250**. | v1.1.1 |
...
| Priority | Issue | Summary | Target |
|---|---|---|---|
| ~~**P1**~~ | ~~Rust engine ignores `USE_TZ`/`TIME_ZONE` (#2209)~~ ✅ **PR #2218** | Every rendered timestamp is off by the UTC offset. Reproduced end-to-end through `LiveView.render()`: a UTC `23:30` renders `2026-08-22 23:30` where Django renders `19:30` — 4 hours, in the **default scaffold config** (`scaffolding/templates.py:171` sets `USE_TZ = True`). No timezone conversion exists anywhere in `python/djust/` or `crates/`; the only `chrono::Local` is `renderer.rs:1208` (`{% now %}`). Affects both value shapes (`.isoformat()` eager dict, `str()` sidecar). **Shipped 2026-08-25.** Fixed in Rust at format time via `chrono-tz`, with the zone pushed per render as a **thread-local** — mirroring Django's own `timezone._active`, which is what makes per-request `timezone.activate()` work and what keeps two `sync_to_async` worker threads from interleaving. A fixed per-render offset was measured and rejected: one render can straddle a DST boundary. tzdata cost measured before adopting — **+1.16 MB raw, +136 KB compressed**. Also implements the five timezone format codes plus `U`, which had been rendering as their own letter (`H:i T` → `19:30 T`). Two corrections to the issue's own premises: the "naive fix costs a GIL crossing" reasoning is wrong (the serializer is already Python) — the real objection is that it corrupts transported state; and the fix needed TWO render paths, not one (#1646, found by grepping the shape rather than the symbol). Surfaced #2216, #2217, #2219. | v1.1.1 |
| ~~**P2**~~ | ~~`date`/`time` filters diverge on bare `date` / `time` objects (#2216)~~ ✅ **PR #2226** (the `time` half) | `{{ v|time:"H:i" }}` on a `datetime.time` **returns its input verbatim** (`23:30:00` where Django gives `23:30`) — the exact class #2203/PR #2208 fixed for datetimes, still live for a different type, because `format_date`'s parse list carries four datetime shapes and one date-only shape and no time-only shape. A `TimeField` through `|time` is ordinary. The `date` half is a fail-soft divergence (djust renders midnight where Django raises `TypeError`) that Rust cannot replicate without type information the serializer discards, and may be correct-as-designed — it wants a documented decision, like the `{% url %}` row above, not a patch. **`time` half shipped 2026-08-25.** Django's rules were enumerated by running all 38 format characters against a `datetime.time`, because they split three ways the docs do not describe: the time codes format, the TIMEZONE codes empty **in place**, and any DATE code empties the **whole** render. The tz rule is also the opposite of the naive-DATETIME rule one line away (#2209), so getting it wrong in either direction was possible — pinned both ways. Fixed lowercase `a` too (`am` → `a.m.`), found by the same differential on datetimes. The bare-`date` half stays UNCHANGED by decision: Rust cannot tell a `date` from a midnight datetime because the serializer discards the type, so it wants a fail-soft policy call like the `{% url %}` row, not a patch. Surfaced #2227 (a third instance of the same parse-shape class, in `timesince`/`timeuntil`). | v1.1.1 |
| ~~**P2**~~ | ~~`timesince`/`timeuntil` parse RFC3339 only (#2227)~~ ✅ **PR #2229** | Third instance of the parse-shape class behind #2203 and #2216: both filters call `DateTime::parse_from_rfc3339` and nothing else, while `format_date` has grown four datetime shapes and a time-only shape. A **naive** datetime — the normal shape under `USE_TZ = False` — does not parse. Each time the list was extended for the filter in front of us and the neighbouring filters with their own parse went unchecked (#1646 on the datetime-parsing axis). Cure is one shared parse helper, not a third correct copy. Not reproduced end-to-end yet: the failure may be a 500 rather than the silent echo its siblings had, which would change the severity — confirm first. **Shipped 2026-08-25.** Confirmed as the SILENT ECHO, not the 500 the issue guessed — a raw ISO timestamp rendered into the page. Cured with one shared `parse_serialized_datetime` rather than a third correct copy, so the chain (#2203 → #2216 → #2227) ends structurally. Reproducing first also caught a defect the parse fix would have shipped **underneath itself**: a naive value compared against `Utc::now()` where Django uses naive local `datetime.now()` — two hours old read as six in a UTC-4 zone. Making a broken path reachable is its own risk. Surfaced #2228 (the output shape diverges on every input, including values that always parsed). | v1.1.1 |
| ~~**P2**~~ | ~~`timesince`/`timeuntil` output shape diverges from Django (#2228)~~ ✅ **PR #2230** | Separate from #2227's parse, and wrong on **every** input including the aware values that always parsed. Three divergences: Django joins number and unit with **U+00A0** (djust uses a plain space — a test written with an ordinary space passes while shipping the wrong byte); Django shows **two adjacent units** (`3 days, 5 hours`) where djust shows one; and Django's smallest unit is the **minute** (`0 minutes` under a minute) where djust reports seconds. Django's month/year arithmetic is also **calendar-aware**, not fixed seconds, so it drifts near boundaries even once the shape is right. The unit names are `ngettext` lookups, tying full parity to the catalog work — decide whether to ship English-only first. The two filters still carry near-identical 30-line formatting blocks; collapse them in the same change (#1646). **Shipped 2026-08-25 — the chain #2203 → #2216 → #2227 → #2228 is closed.** Ported from `django/utils/timesince.py` rather than approximated, including the calendar-aware year and month arithmetic (Django's own docstring cases: 393 and 397 days are both exactly '1 year, 1 month'). Django's February-28 `MONTHS_DAYS` quirk is reproduced deliberately — parity is the point. Verified by a **1,600-case randomized differential** against `django.utils.timesince`: 0 diffs. Two of my own errors were settled by running Django rather than reasoning, and they resolved in OPPOSITE directions (once the test was wrong, once the code was). Translated unit names are still out — `TIME_STRINGS` are `ngettext` lookups, so full i18n parity needs the catalog work. | v1.1.1 |
| ~~**P3**~~ | ~~Nine Django date format codes unimplemented (#2217)~~ ✅ **PR #2231** | `S w t L b f u c r` fall through `format_date`'s catch-all and render as **their own letter**: `{{ v|date:"jS F Y" }}` gives `22S August 2026`. `S` bites hardest — `jS` is a common idiom and the output is visibly broken. Quiet because rendering an unknown character as itself is *also* correct for a literal, so an unimplemented code is indistinguishable from a literal one without a differential against Django. Mechanical (1–3 lines each); the value is pinning all 26 codes in one table so the next gap cannot hide (v1.0.0rc4 retro finding #1). **Shipped 2026-08-25 — all 38 codes now match Django** (3,000-case randomized differential, 0 diffs). A TENTH code was already implemented and already wrong: `N` is Associated Press style, so March–July are spelled out and September is `Sept.` — half the year, with an existing unit test pinning the wrong answer. The sampling lesson landed **three times inside the PR built to prevent it**: `N` plus two gate-off mutations (`W` as `ordinal/7+1`, `o` as the calendar year) all survived a table that enumerated every CODE while sampling three VALUES per code. Enumerate-every-variant applies to every axis, not the one you noticed. Localized month/day names remain open as piece 2 of #2221. | v1.1.1 |
| ~~**P2**~~ | ~~`simple_live_view` cannot render at all (#2219)~~ ✅ **PR #2233** | `get_context_data` walks `dir(self)` and getattrs each name, reaching Django's `View.as_view` — a `classonlymethod` that raises `AttributeError` on any **instance** — so every render fails before a template, and `except Exception` turns it into a generic message. Unexported, untested, and its class is named `LiveView`, so grepping `SimpleLiveView` finds nothing and it reads as unused when it is merely unfindable. Only outside reference is `pyproject.toml:330` listing it as an ADR-023 strict-typing island — type-checked, never executed. Wants a **decision** (remove it, or fix + test + export under a non-colliding name), not a one-line `try/except`. Found auditing render paths for #2209; PR #2218 wired it to the shared handoff anyway and its test asserts the `AttributeError`, so that test reddens the moment this is fixed. **Shipped 2026-08-25.** Fixed rather than removed, and the class renamed `SimpleLiveView` so it stops shadowing `djust.LiveView` — the name collision is *why* it stayed invisible for two PRs. The fix is the `AttributeError` guard, not a name exclusion: `as_view` is deliberately NOT in the plumbing set, because listing it in both places made the two mechanisms shadow each other and re-introducing the ORIGINAL bug left the suite green (#2129). Also fixed a cross-test leak **this release introduced**: a reset fixture using `translation.deactivate_all()` left `NUMBER_GROUPING=0` for every later test in the worker. | v1.1.1 |
| ~~**P1**~~ | ~~The Rust engine is locale-blind (#2221)~~ ✅ **PR #2222** (piece 1 of 3) | **Filed, and the Stage-4 decision is that it does NOT fold into #2209.** Probing widened it twice. It is **not a German-projects problem**: `USE_THOUSAND_SEPARATOR` applies regardless of language, so Django renders `1,234.50` where djust renders `1234.50` **in the default en-us config**. And it is **not confined to `floatformat`**: bare `{{ n }}` and `{{ i }}` are affected too — every rendered number in every template. It does not fold into #2209 because the fix shapes are inverses: timezone needed a self-contained DATABASE in Rust (`chrono-tz`, +136 KB) and nothing from Python but a name; locale needs **Django's own data** (`conf/locale/*/formats.py`, `.mo` catalogs), so the parameters must be resolved in Python and pushed down — reusing the per-render mechanism #2209 already built rather than inventing a second. Split into three: **(1) number separators** — the filed complaint, diverges in en-us, biggest surface, cheapest (3 strings per render); **(2) month/day names** — finite vocabulary (~40 strings), pushable per language; **(3) locale `DATE_FORMAT`** — blocked on bare datetimes rendering as raw ISO strings, its own issue. Do (1). **Piece 1 shipped 2026-08-25** — numbers now match Django across en-us/de/fr, including French's U+00A0. The fold decision recorded above held: the fix shape is the INVERSE of #2209's (Python resolves, Rust applies), because locale is Django's own data while a tz database is self-contained. Applied at the variable-OUTPUT site, not in `Display` — which is also the `{% if x in dict %}` lookup key, so a separator there would have silently missed every such lookup; pinned by a test. `USE_L10N` deliberately unread (inert since Django 5.0). **Pieces 2 (month/day names) and 3 (per-locale DATE_FORMAT) remain open** — both need translation catalogs rather than format parameters. | v1.1.1 |
| ~~**P2**~~ | ~~`live_redirect` hardcodes the DB session backend (#2210)~~ ✅ **PR #2220** | `runtime.py:3978` and `websocket.py:3313` import `sessions.backends.db.SessionStore` directly when synthesizing a request; `settings.SESSION_ENGINE` is never read anywhere in the package. A cache-backed project gets a store that reads a `django_session` row which does not exist → **empty session** on the re-auth path, plus a DB round trip it configured its way out of. `scope["session"]` is correct, so only the synthesized-request path is wrong — which is why it went unnoticed. **Shipped 2026-08-25.** The issue's own request to reproduce before fixing paid three times over. The symptom was worse than filed — a cache-only project has no reason to run the sessions migration, so the likelier shape is `OperationalError: no such table: django_session`, raised lazily wherever the view first READS the session rather than where it was built. The site count was four, not two: `testing.py` has two more, and a `LiveViewTestClient` on a different session engine than the production paths it stands in for is its own trap. And the `signed_cookies` recommendation was wrong — reads work (the "key" IS the signed payload), only writes cannot persist, so it warns once and keeps the reads rather than refusing. All four converge on one `utils.build_session_for_request`, pinned structurally. | v1.1.1 |
...
| ~~**P1**~~ | ~~A third Python→Value converter checks `i64` before `bool` (#2212)~~ ✅ **PR #2213** | PyO3 0.29 extracts a Python `True` as `i64` `1` (its own `test_i64_bool` asserts this), so any converter trying `i64` first has a **dead `bool` arm**. Three such converters exist; #2211 fixed one, and `djust_live/src/lib.rs:2346` still checks `i64` at `:2346` before `bool` at `:2352` — a bool crossing that path comes back as Python `1`. Split out of the #2211 review to keep that PR scoped (#1079). Same #1646 shape as the rest of this bucket: one invariant, three implementations, and the compiler cannot see a divergence in any of them. **Trace the callers before scoping severity** — the issue does not, and that decides whether this is cosmetic or a live wire-format bug. **Shipped 2026-08-25.** Severity resolved as a live wire-format bug, not cosmetic: `serialize_python_value` is the JIT template-serialization path, so every `BooleanField` on a model reached Rust as `Value::Integer(1)` and rendered `1`/`0` where Django renders `True`/`False`. The count was wrong too — a structural grep found **six** such converters, not three, and a review gate-off proved **five have no behavioural test at all** (reverting #2211's fix left the suite green). Guarded structurally rather than by hand-enumeration. Surfaced #2214. | v1.1.1 |
| **P2** | `DecimalField` reaches the client as a lossy float (#2214) | `extract::<f64>()` sits ABOVE the `Decimal`/`UUID` stringify branch in the same `serialize_python_value` that #2212 fixed, and PyO3's `f64` extraction honours `Decimal.__float__` — so that branch is **dead for `Decimal`** and every `DecimalField` crosses as a binary float: `Decimal('12345678901234567890.123456789')` arrives as `1.2345678901234567e+19`. `DecimalField` is what Django projects use for money, and binary floats are precisely what it exists to avoid. The function's own doc-comment claims these convert to strings — false about the code it sits on (#1867). Second instance of the #2212 class in one function; the fix should widen the structural guard to the general rule (**no `type_name ==` special case below a permissive `extract::<T>()`**) rather than pin one more pair. **Guard shipped 2026-08-25 (PR #2224); the issue stays OPEN by design.** The one-line fix it suggests was measured to REGRESS two template behaviours — this value goes back into the template context, not only onto the wire, so `{{ p|floatformat }}` gives 19.99 instead of 20.0 and `{% if p > 10 %}` takes the false branch once it is a string. Needs a decision (a Decimal-aware `Value` variant vs an accepted precision limit), not a patch; three options are on the issue. What shipped is the half the issue itself called more valuable: the sweep generalised to *no `type_name ==` special case below an extract that swallows it*, with a **measured** capture table that caught an error in its own first draft (PyO3's i64 uses `__index__`, not `int()`). A strict xfail is the tripwire: fixing this makes the test XPASS. | v1.1.1 |
| ~~**P2**~~ | ~~The Django-template backend never pushed per-render settings (#2223)~~ ✅ **PR #2225** | The caller set #2209 and #2221 pinned was missing a **top-level** path: a plain Django template through `DjustTemplateBackend` rendered `1234567|23:30` where Django renders `1,234,567|19:30`. Survived two PRs because the thread-local persists — a worker that already served a LiveView carries a correct environment into later backend renders, so it only shows on a thread that has not. Wired at the top-level entry, not inside `_rust.render_template*`: measured at ~12µs against a ~15µs render, and most calls are nested ones that already inherit. Two of my own test claims failed their gate-off and were corrected in the test file — the fresh-thread fixture is hygiene rather than a detector, and one of three cases cannot detect the defect at all. | v1.1.1 |
| **P2** | The #1882 wire-version pollution flake is still live (#2215) | `test_time_travel_jump_recovery_version_is_current` failed in CI on PR #2213 with `arming event must be 2, got 3` — the exact counter-bump signature `python/djust/test_isolation.py:16` documents as **cured** by `reset_djust_globals`. Trigger was adding two cases to an unrelated test file: no production code on that path changed, but the re-shard was enough. New narrowing this sighting adds — the mount baseline assertion **passed** and the next one failed, so the bump happened *during* the test on an already-clean connection, pointing at a **live producer** (lingering task / watcher / tick loop) rather than buffered state. A pre-test reset cannot cure that. Did not reproduce locally (3/3 green full CI-way, 3/3 targeted); needs the runner's 4-worker sharding, so pin `-n 4`. Concrete sighting for #2187; likely duplicates it. | v1.1.1 |
...
| Priority | Issue | Summary | Target |
|---|---|---|---|
| ~~**P2**~~ | ~~`main` has no required status checks (#2163)~~ ❌ **closed, not planned** | Branch protection sets `required_status_checks: null`, so the entire `test-summary` aggregate — rust, python, javascript, browser-smoke, nav-hooks-guard, security-tests, demo-checks, benchmarks — can be red and the merge button stays live. #1713 one level up: being in `needs` is not the same as gating. Blocked on a prerequisite: `test.yml` carries `paths-ignore` for `**.md`/`docs/**`/`CHANGELOG.md`, so a required `test-summary` would leave docs-only PRs permanently unmergeable. **Closed 2026-08-25 as a POLICY decision, not a blocked one.** The prerequisite this row cites is already solved — `test-summary-docs-shim.yml` was built precisely so the check can be required without stranding docs-only PRs, and it skips on mixed PRs so it can never green-wash. So nothing technical stands in the way. It stays off because `--admin` is the working convention and bypasses required checks anyway: with effectively one committer the gate would be theatre on the path actually used. The FINDING stands and is worth keeping — the aggregate is not a merge gate, and #1713's rule (being in `needs` is not gating) is the thing to check whenever a CHANGELOG says something "now gates X". Revisit if the repo gains contributors or `--admin` stops being routine. | v1.1.1 |
...
| ~~**P2**~~ | ~~RETRO.md: 121 unchecked Open Items reference already-closed issues (#2200)~~ ✅ **PR #2232** | Sibling of #2181, one file over: 121 `- [ ]` rows in RETRO.md whose referenced issues are ALL closed — **including the row proposing to automate this check**, which is the finding. RETRO.md currently carries 197 unchecked items total, so roughly 60% of the open-work signal in the project's own retro ledger is noise. Fix mechanically (resolve each row's issues, strike the settled ones) and then decide whether a pre-commit check is worth it — note #2181 and this one are the same class, so a single checker could cover both files. **Shipped 2026-08-25 — 168, not 121** (more issues closed in between, a dozen of them in this drain). Fixed by EXTENDING `scripts/check-action-tracker.py` rather than adding the sibling script the issue proposed: it already owned the batched fetch, `--fix`, the Makefile target and the tests, and RETRO.md's two structures drift identically. A numbering COLLISION nearly wrote a false citation — `Action Tracker #329 (GitHub #2142)`, and tracker numbers collide with real issue numbers, so a first pass reported #2142 as closed by #329's PR. All 117 citations were verified against GitHub individually before commit (#1197). | v1.1.1 |
...
- **Rust + WASM client patcher (post-1.0, v1.x or v2.x ambitious bet)** — The biggest single Rust opportunity djust hasn't taken yet: replace the shipped client (`client.min.js.gz`, ~64 KB gz — run `make sizes`) with a Rust-compiled WASM patcher. Wire protocol unchanged — just a different patcher implementation on the client side. ~2-3 month project. Wins:
