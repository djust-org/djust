...
~~**Nested form handling (`inputs_for`)**~~ ✅ **Shipped in v0.5.1** — `{% inputs_for formset as form %}...{% endinputs_for %}` template tag iterates a Django `BaseFormSet` with per-row prefixes; `FormSetHelpersMixin` provides `add_row` / `remove_row` event handlers. Free-standing helpers in `djust.formsets`. 13 test cases.
