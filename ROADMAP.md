...
**Rust VDOM diff does not detect attribute changes inside `|safe` HTML blobs ([#783](https://github.com/djust-org/djust/issues/783))** — When a `{{ field_html.field_name|safe }}` expression changes between renders (e.g., `value=""` → `value="Amanda"`), the Rust VDOM diff produces zero patches. Python-side change tracking correctly sends updated context to Rust, but the diff engine treats `|safe` HTML blobs as opaque text nodes and doesn't compare their contents. Affects WizardMixin auto-fill and any pattern that changes form field values via pre-rendered HTML strings. `_force_full_html` does NOT fix this (the Rust diff itself is the problem). Priority: P0 — blocks production use of WizardMixin.
