...
| **P1 ⭐** | **Auto-generated HTTP API from `@event_handler`** ([ADR-008](docs/adr/008-auto-generated-http-api-from-event-handlers.md)) | **v0.7.0 headline feature.** Opt-in `expose_api=True` turns handlers into `POST /djust/api/<view>/<handler>/` endpoints with OpenAPI schema — unlocks mobile, S2S, CLI, and AI-agent callers without duplicating logic. Transport adapter over the existing handler stack (same coercion, permissions, rate limiter) → manifesto principle #4 preserved. | v0.7.0 |
...
### Milestone: v0.7.0 — HTTP API, Navigation, Smart Rendering & AI Patterns

*Goal:* Ship the **auto-generated HTTP API from `@event_handler`** as the headline feature (unlocks mobile, S2S, CLI, and AI-agent callers). Make navigation feel like a SPA. Establish djust as the best framework for AI-powered applications, where "AI" covers both UIs that USE AI and APIs that AI consumes.
...
*Why HEADLINE for v0.7.0:* ADR-008 is a strategic inflection point. Every LLM-agent platform consumes OpenAPI; every mobile team avoids WebSocket-first frameworks; every S2S integration wants plain HTTP. Shipping this makes djust a credible back-end choice for those workloads — not just a reactive-UI framework. It also composes with every other v0.7.0 entry: `dj-activity` background pre-rendering, prefetch-on-hover, and server functions all benefit from having an HTTP transport they can reuse. Scoped per ADR-008 §"Decision" + §"Design sketch"; does NOT include streaming responses (HTTP/2 SSE deferred) or the GraphQL-style batching mentioned in ADR-008 §"Out of scope".
