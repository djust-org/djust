...
~~**Rust VDOM diff does not detect attribute changes inside `|safe` HTML blobs ([#783](https://github.com/djust-org/djust/issues/783))**~~ ✅ — Fixed by PR #779 (container value equality tracking). The root cause was Python-side `id()` comparison missing derived dict changes, not a Rust diff bug. Closed without additional code.
