...
**Streaming initial render** — Chunked HTTP page shell + progressive content. Django's `StreamingHttpResponse` + djust's template engine can emit the `<head>` + `<body>` wrapper immediately, then stream LiveView content as each mount completes. Faster perceived load than full-page wait; competitive with Next.js `renderToPipeableStream`. Integrates with the existing `dj-lazy` hydration path — the server emits a placeholder for each lazy view, streams the real content as each view finishes mounting. ~200 lines Python. *Deferred from v0.6.0rc1 because it touches the HTTP response boundary and needs careful integration testing across ASGI deployments (daphne, uvicorn, hypercorn).*
...
| **Streaming initial render** | Chunked HTTP page shell + progressive content — faster perceived load than full-page wait | **v0.6.1** (deferred from v0.6.0rc1) |
...
| Streaming initial render | — | `renderToPipeableStream` | Not started | **v0.6.1** |
