...
| **P3** | Nine Django date format codes unimplemented (#2217) | `S w t L b f u c r` fall through `format_date`'s catch-all and render as **their own letter**: `{{ v|date:"jS F Y" }}` gives `22S August 2026`. `S` bites hardest — `jS` is a common idiom and the output is visibly broken. Quiet because rendering an unknown character as itself is *also* correct for a literal, so an unimplemented code is indistinguishable from a literal one without a differential against Django. Mechanical (1–3 lines each); the value is pinning all 26 codes in one table so the next gap cannot hide (v1.0.0rc4 retro finding #1). | v1.1.1 |
