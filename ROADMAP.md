...
**Animations & transitions** — *(phases 1 + 2a of 4 shipped in v0.6.0; remaining phases deferred.)* ~~Declarative `dj-transition` attribute for enter/leave CSS transitions with three-phase class application (start → active → end), matching Phoenix's `JS.transition`.~~ ✅ **Shipped (v0.6.0)** — `41-dj-transition.js`, 7 JSDOM tests, guide updated. ~~`dj-remove` (exit animations before element removal).~~ ✅ **Shipped (v0.6.0)** — `42-dj-remove.js`, hooks into 5 VDOM-patch removal sites, 10 JSDOM tests. Still to ship: FLIP technique for list reordering, `dj-transition-group` (React `<TransitionGroup>` / Vue `<transition-group>` equivalent), Skeleton/shimmer loading-state components. *(View Transitions API integration was promoted to v0.5.0.)*
