...
**Transport-conditional API returns (`_api_request` flag + `@api_returns` decorator) — P2 follow-up to ADR-008** — Handlers serving both WebSocket and HTTP API consumers have conflicting needs: WS callers need state mutation only (return value ignored, VDOM diff handles rendering); HTTP API callers need actual data in the response. Returning serialized data unconditionally wastes DB queries and serialization on every WS keystroke. Fix: `dispatch.py` sets `view._api_request = True` before handler invocation (1 line). A `@api_returns(serializer_fn)` decorator wraps the handler — calls `serializer_fn(self)` only when `_api_request` is True, returning serialized data as `{"result": {...}}`. On WS, the decorator is a no-op. Pattern discovered in NYC Claims project (djust-org/djust ADR-008 §"Open questions" update). Implementation: ~10 lines in `dispatch.py` + optional framework-level decorator.
