...
- **`{% inputs_for %}` nested-formset template tag + `FormSetHelpersMixin` (v0.5.1 P2)** — Django formset/inline-formset patterns with LiveView-aware wrappers. The new `{% inputs_for formset as form %}...{% endinputs_for %}` block tag iterates a Django `BaseFormSet` and exposes each child form as a context variable with its correct per-row prefix intact; `inputs_for_loop.counter` / `counter0` / `first` / `last` match Django's `{% for %}` conventions. Paired with `djust.formsets.FormSetHelpersMixin` which provides opt-in `add_row` / `remove_row` event handlers keyed on `formset_classes = {"addresses": AddressFormSet}` — reads `dj-value-formset` / `dj-value-prefix` from the event. Free-standing `add_row()` / `remove_row()` functions also exported for custom handlers. Add caps at `formset.absolute_max`; remove uses Django's standard `DELETE=on` flag (non-destructive — visible to `formset.deleted_forms` during validation). 13 test cases. (`python/djust/templatetags/djust_formsets.py`, `python/djust/formsets.py`)
