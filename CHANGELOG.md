...
- **Package consolidation Phase 4: `djust-theming` folded as `djust[theming]`** — `djust-theming` (49,105 LOC, 176 files) merged into `python/djust/theming/` as an optional extra (`pip install djust[theming]`). Includes CSS theming engine, design tokens, 96 HTML templates, 9 CSS/JS static files, management command (`djust_theme`), 4 template tag modules, and gallery sub-package. 749+ theming tests merged.
- **Package consolidation Phase 5: `djust-components` folded as `djust[components]`** — `djust-components` (~100K LOC, 371 files) merged into `python/djust/components/` as an optional extra (`pip install djust[components]`). Includes 170+ UI component classes, 6 template tag modules, management command (`component_gallery`), descriptors, mixins, and rust_handlers.py for Rust template engine integration. Extra deps: `markdown>=3.0`, `nh3>=0.2`. `pip install djust[all]` now includes all packages.
