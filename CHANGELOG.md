...
- **New consolidated `djust_audit` command guide** — `docs/guides/djust-audit.md` documents all four modes of the command (default introspection, `--permissions`, `--dump-permissions`, `--live`), every CLI flag, CI integration examples, and exit-code conventions. Cross-linked from `docs/guides/security.md`.
