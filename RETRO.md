...
| 18 | Broaden dep-extractor correctness harness matrix (Spaceless, standalone CustomTag, nested With, standalone Block, ReactComponent, RustComponent) | PR #785 | #786 | Closed | Resolved by PR #869 |
| 19 | Extract filter-arg vars as deps in `extract_from_variable` | PR #785 | #787 | Closed | `{{ a\|default:fallback }}` drops `fallback` |
| 20 | Slot-in-for-loop test coverage (Risk 1 from plan) | PR #788 | #789 | Closed | Resolved by PR #862 |
| 21 | `{% render_slot slots.col.0 %}` dotted-path end-to-end test | PR #788 | #790 | Closed | — |
| 22 | Unrelated ruff reformats of 5 test files (stashed during PR #788) | PR #788 | #791 | Closed | Chore |
| 23 | `assign_async` concurrent same-name cancellation semantics | PR #792 | #793 | Closed | In-flight task can overwrite with stale data |
| 24 | `logger.debug` ping on non-AsyncResult value in `{% dj_suspense await= %}` | PR #792 | #794 | Closed | Chore |
| 25 | `suspense.py:138` redundant check + CHANGELOG test-count nit | PR #792 | #795 | Closed | Chore |
| 26 | Variable-height virtual list items via ResizeObserver | PR #796 | #797 | Closed | ~200 LOC, v0.5.1 candidate |
| 27 | `teardownVirtualList` should restore `originalChildren` | PR #796 | #798 | Closed | Resolved by PR #866 |
| 28 | Server-side `stream_append(limit=N)` should trim inserts before sending | PR #796 | #799 | Closed | Resolved by PR #866 |
...
| 30 | `stream_prune` `.children` filter redundancy in `17-streaming.js` | PR #796 | #801 | Closed | Chore |
| 31 | Block-handler loader access (deferred item 2b from PR #802) | PR #802 | #803 | Closed | ~40 LOC Rust |
| 32 | Parent-tag propagation for nested custom-tag handlers (deferred item 2c) | PR #802 | #804 | Closed | — |
| 33 | Warn when `register_assign_tag_handler` returns non-dict | PR #802 | #805 | Closed | ~5 LOC |
| 34 | Extend `Context::resolve` to `Node::For` iterables over Model instances | PR #802 | #806 | Closed | `{% for user in users %}` over QuerySet doesn't walk getattr |
| 35 | `PostgresNotifyListener` event-loop binding across `async_to_sync` | PR #807 | #808 | Closed | Resolved by PR #875 |
| 36 | `untrack()` helper for `@notify_on_save` signal receiver cleanup | PR #807 | #809 | Closed | Resolved by PR #874 |
| 37 | NOTIFY payload size guard (PostgreSQL 8000-byte limit) | PR #807 | #810 | Closed | Resolved by PR #867 |
| 38 | `reset_for_tests` should await task cancellation | PR #807 | #811 | Closed | Resolved by PR #867 |
| 39 | Regression test for views without `NotificationMixin` | PR #807 | #812 | Closed | Resolved by PR #867 |
| 40 | Document 100 ms render-lock timeout behavior in db_notify handler | PR #807 | #813 | Closed | Resolved by PR #867 |
| 41 | `dj-ignore-attrs` should cover morph-path attribute sync/removal | PR #814 | #815 | Closed | Contract-scope gap in 12-vdom-patch.js |
| 42 | `dj-ignore-attrs` CSV edge cases (empty/whitespace/trailing comma/double comma) | PR #814 | #816 | Closed | — |
| 43 | Namespacing `AttributeError` fallback regression test | PR #814 | #817 | Closed | — |
| 44 | Escape namespaced name in `data-hook` attribute (defense in depth) | PR #814 | #818 | Closed | Chore |
| 45 | Pre-signed S3 PUT URLs (client-direct S3 upload bypassing djust) | PR #819 | #820 | Closed | Feature |
| 46 | Resumable uploads across WebSocket disconnects | PR #819 | #821 | Closed | Feature, v0.6+ |
| 47 | First-class GCS and Azure Blob UploadWriter subclasses | PR #819 | #822 | Closed | Feature |
| 48 | `BufferedUploadWriter._finalized` flag is dead code | PR #819 | #823 | Closed | Resolved by PR #864 |
| 49 | Client "stop sending" signal after size-limit abort (BufferedUploadWriter backpressure) | PR #819 | #824 | Closed | — |
| 50 | Document JSON-serializability constraint on `UploadWriter.close()` return | PR #819 | #825 | Closed | — |
| 51 | Document that `<script>` in swapped `<main>` won't execute (SW + #814 interaction) | PR #826 | #827 | Closed | Resolved by PR #860 |
| 52 | `DjustMainOnlyMiddleware` should early-return on 4xx/5xx responses | PR #826 | #828 | Closed | — |
| 53 | `registerServiceWorker` duplicate-call guard | PR #826 | #829 | Closed | — |
| 54 | Middleware should match `text/html` with charset variants | PR #826 | #830 | Closed | — |
...
| 83 | `_registry.py` F401 unused-import alerts may need explicit `# noqa` post-rescan | PR #929 | #1025 | Closed | No issue filed; revisit after CodeQL rescan |
...
| 94 | Bump `.pre-commit-config.yaml` ruff-pre-commit from v0.8.4 to current | PR #940 retro | #948 | Closed | Local ruff disagreements with pinned hook cause spurious format churn |
| 95 | tag_input CSV serialization ambiguous for commas-in-values | PR #939 retro | #949 | Closed | Escape, multiple inputs, or JSON-encode — decide and document |
| 96 | pipeline-run outer loop should verify retro comment before `completed_at` | PR #946 retro | #950 | Closed | Retro dropout caught in drain — add artifact gate |
| 97 | dj-virtual variable-height: data-key-based cache survives reorders | PR #947 retro | #951 | Closed | Currently index-keyed; reorders bind heights to wrong items |
| 98 | dj-virtual variable-height guide page | PR #947 retro | #952 | Closed | Tuning estimated-height, scrollbar-jump tradeoffs, data-key story |
...
| 102 | v0.6.0 or v0.7.0 decision: breaking rename of framework-internal attrs to `_*` prefix | Retro v0.5.7 / PR #957 | #962 | Closed | #762 shipped non-breaking filter; rename still on table |
| 103 | Weekly real-cloud CI matrix job for S3 / GCS / Azure upload writers | Retro v0.5.7 / PR #958 | #963 | Closed | All SDK tests are mocked; no real-cloud end-to-end |
| 104 | Document `key_template` UUID-prefix convention for `s3_events.parse_s3_event` | Retro v0.5.7 / PR #958 | #964 | Closed | Silent fallback to full key otherwise |
...
| 139 | pipeline-retro Stage 2 (read per-PR retros) reinforcement | Retro v0.8.0 | #1054 | Closed | New (reconcile 2026-04-25) |
...
| 142 | make roadmap-lint — automate ROADMAP-vs-codebase grep | Retro v0.5.0 feature rollout | #1057 | Closed | New (reconcile 2026-04-25) |
...
| 145 | Dogfood pass for new CLI tools before commit | Retro v0.5.1 | #1060 | Closed | New (reconcile 2026-04-25) | **Validated in v0.8.2 (PR #1074)** — Validated under fire — caught the dataclass vs dict bug at Stage 5 in PR #1074. Discipline empirically working; first real-world payoff. |
| 146 | Pre-push hook for `noqa: F822` in `__all__` patterns | Retro v0.7.0 | #1061 | Closed | New (reconcile 2026-04-25) |
| 147 | Centralize tech-debt issue queue around Action Tracker rows | Retro 2026-04-25 reconcile | #1062 | Closed | New (reconcile 2026-04-25) |
...
| 162 | Demand bit-exact runnable repro before posting "root cause confirmed" on a multi-reopen issue | Retro v0.8.4 / PR #1086 (process) | #1389 | Closed | New — posted 3 "root cause" / "smoking gun" comments on #1081 based on framework-side theory testing; all three were wrong. The actual fix landed only after gaining direct project access. Process rule: on any issue with N≥2 reopens, refuse to post a root-cause claim without a runnable script that reproduces against the user's exact environment. Add to `pr/feedback/` triage checklist. |
| 163 | Split-foundation pattern for high-blast-radius features (canonicalize) | Retro v0.8.6 / View Transitions arc | #1122 | Closed | Validated 3× now (View Transitions PR-A/PR-B, plus #1098 fix between). Add to CLAUDE.md / ADR template. |
...
| 227 | dj-if + dj-key boundary-reorder limitation in keyed VDOM diff | PR #1365 Stage 13 (deferred) | #1366 | Closed | When non-boundary siblings carry `dj-key` AND reorder within their relative slot, position-based pairing of non-boundary children can produce suboptimal patches. Production templates don't typically reorder elements across `{% if %}` boundaries. Defer to v0.10 polish. Suggested fix: extend the pre-pass to delegate to `diff_keyed_children` when any non-boundary children have `dj-key`. |
...
| 278 | Empirical-canary fixture for `assert_http_ws_djid_parity` (Action #1468 spirit) | PR #1653 (#1642 retro) | #1654 | Closed | Harness passes for every shape today, so catch-power rests on the path-differential argument; add a synthetic divergence fixture so it can't go green-but-toothless. |
| 279 | Strengthen drain guards: scaffold deploy-path integration test + `_arm_recovery` caller-count test + single `isSignificantChild` predicate | Retro v1.0.0rc14 (PRs #1651/#1652/#1649) | #1655 | Closed | Three non-blocking test/code-hardening deferrals consolidated. |
...
| 299 | Consumer-owned monotonic VDOM send-version (removes recovery round-trip on `html_update`) | PR #1789 (#1785) follow-up | #1788 | Closed | Deferred — optimization not bug post-#1785; carries drift risk. |
...
| 307 | System check to warn when `dj-view`/`dj-root` is on a table-section element (`<tbody>`/`<tr>`/…) — silently foster-parented to garbage at render time | Retro v1.0.7-1 / #1827 investigation | #1837 | Closed | Surfaced closing #1827: a `<tbody dj-view>` template renders as `<html><body>text</body></html>` (all rows dropped), no error. The actionable DX fix the #1827 diff_html flattening pointed at. |
...
| 311 | Test-ordering pollution: `tests/unit/test_demo_views.py::TestDemoRegistration` — 4 urlconf-resolution failures under full `-n auto` | PR #1861 / v1.0.8-1 retro | #1862 | Closed | Pre-existing; surfaced during T1-A. Passes in isolation; needs the polluter found (urlconf state leak). |
| 312 | `_get_project_app_dirs()` returns 0 dirs when `manage.py check` runs from inside the djust repo tree (the `/djust/` path filter) — blinds S009/S011 dogfooding | PR #1864 / v1.0.8-1 retro | #1865 | Closed | Pre-existing; makes in-repo dogfood of the new checks see 0 app dirs. Dogfood worked via the demo project instead. |
| 313 | Per-model `djust_serializable_fields` allowlist can re-expose the sensitive-field floor (`password`/`is_superuser`/`is_staff`) — allowlist wins over `_ALWAYS_EXCLUDED_FIELDS` | PR #1867 / v1.0.8-1 retro | #1868 | Closed | Surfaced by writing SECURE_DEFAULTS.md (`serialization.py:362`). Doc now states accurate precedence + WARNING; code-hardening question (make floor unconditional?) tracked here. |
...
| 318 | dj-virtual deeper server-side reconcile — automatic `stream_append`→`__djVirtualItems` wiring, VDOM-differ `dj-virtual` awareness (keyed mid-list inserts/removals), out-of-window finalize-patch landing, + a `djustDebug` warning | PR #2018 / v1.1.0-5 retro | #2017 | Closed | Deferred from #1988/#1989 per #1079 scope discipline. **Closed in 1.1.1 (PR #2197)** — iteration 3 shipped, `virtual_keyed_ops` defaults ON. It stayed open longer than the cited blockers implied: the browser gate could not produce evidence because the CONTROL arm was as broken as the test arm, and #2185 + #2194 had to ship before an A/B meant anything. Items 1 and 5 of the issue (stream-op wiring, patch-miss diagnostic) shipped earlier; out-of-window finalize-patch landing remains open. |
...
| 324 | dj_transition `re-runs the sequence` JS test still uses polling `waitForClass` — real-rAF flake (#1830 class) | PR #2077 / v1.1.0-11 retro | #2081 | Closed | Surfaced during the #2070 drain (cost one CI cycle); the case wasn't converted to the controllable-rAF stub PR #1839 applied to its sibling. Fix: drive `flushFrame()` + assert an ordering invariant + gate-off sibling. |
...
| 326 | One source of truth for the client-size figures — three copies describing two artifacts, only one checked | Retro v1.1.0-13 / PR #2134 | #2138 | Closed | README pair is enforced against a ±3 KB band; the CLAUDE.md pair was unchecked and had drifted >2× (~87 KB claimed vs ~184 KB actual). Prefer generating the figures at build time over adding a second checker. Note `client.min.js.gz` is gitignored, so the check depends on a local build. |
| 327 | Detect a red `main` proactively instead of discovering it via a failed push | Retro v1.1.0-13 / PR #2134 | #2139 | Closed | Three failing tests on `main` made the pre-push hook reject every branch, including ones unrelated to the failure; took three failed pushes to diagnose. Same argument as #2124 — a persistently red suite trains people to read red as noise. Wants a scheduled main-suite check AND a hook that distinguishes pre-existing merge-base failures from branch-caused ones. |
