...
# Count JS test/it() declarations. Matches a test declared in STATEMENT
# position — ``it(``, ``test(``, ``it.only(``, ``it.each(``, etc. at the
# start of a line (any indentation) — which is how every declaration in
# tests/js/ is written (#2839 corpus scan: 178 files, 1856 tests, verified
# against ``vitest run``).
#
# The pre-#2839 regex matched any whitespace/;/{-preceded ``it|test`` + ``(``
# ("deliberately loose"), which also matched PROSE: a comment sentence like
# "...block below it (`requiredKey`)..." counted as a test, rejecting correct
# changelog claims (#2839) and letting inflated ones pass. Anchoring on line
# start removes the prose class without dropping any real declaration in the
# corpus.
#
# Known limitations (accepted, pre-existing): a test declared mid-line (a
# one-liner ``describe('x', () => { it(...) })`` suite) is not counted, and
# tests generated by a loop contribute one declaration for the loop body, not
# one per iteration. No such file exists in tests/js/ today; the two files
# that generate tests dynamically are counted at their declaration count.
JS_TEST_FN_RE = re.compile(
    r"^[ \t]*(?:it|test)(?:\.(?:only|skip|each))?\s*\(",
    re.MULTILINE,
)
