...
class LiveView(
    ...
    # ============================================================================
    # COMPONENT CONTEXT SHARING — React Context API equivalent (v0.5.1)
    # ============================================================================

    def provide_context(self, key: str, value: Any) -> None:
        """Expose a value to descendant components under ``key``.

        Descendant ``LiveComponent`` instances (or nested ``LiveView``s in
        unusual compositions) can read the value with
        :meth:`consume_context`. The context is scoped to the current render
        tree and is NOT persisted across renders — re-provide values each
        ``mount()`` or ``get_context_data()`` if they should survive.
        """
        providers = self.__dict__.setdefault("_djust_context_providers", {})
        providers[key] = value

    def consume_context(self, key: str, default: Any = None) -> Any:
        """Return the value provided under ``key`` by an ancestor.

        Search order: this view's own ``_djust_context_providers``, then any
        ``_djust_context_parent`` chain (set when a component is attached to
        a parent via the component registry). Returns ``default`` if no
        provider is found.
        """
        node = self
