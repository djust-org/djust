...
class TenantAwareRedisBackend(TenantAwareBackendMixin, PresenceBackend):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "redis is required for TenantAwareRedisBackend. "
        ...
        try:
            self._client.ping()
            logger.info(
                "TenantAwareRedisBackend connected for tenant %s",
                tenant_id
            )
    ...
    def join(self, presence_key: str, user_id: str, meta: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.debug(
            "User %s joined tenant %s presence %s",
            user_id, self._tenant_id, presence_key
        )
        return record
    ...
    def leave(self, presence_key: str, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        if record:
            logger.debug(
                "User %s left tenant %s presence %s",
                user_id, self._tenant_id, presence_key
            )
        return record
    ...
    def cleanup_stale(self, presence_key: str) -> int:
        ...
        logger.debug(
            ...
            len(stale), self._tenant_id, presence_key
        )
        return len(stale)
...
class TenantAwareMemoryBackend(TenantAwareBackendMixin, PresenceBackend):
    ...
    def join(self, presence_key: str, user_id: str, meta: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Set heartbeat
        heartbeat_key = (self._tenant_id, presence_key, user_id)
        self._heartbeats.setdefault(self._tenant_id, {})[f"{presence_key}:{user_id}"] = now
        
        logger.debug(
            "User %s joined tenant %s presence %s (memory)",
            user_id, self._tenant_id, presence_key
        )
        return record
    ...
    def cleanup_stale(self, presence_key: str) -> int:
        ...
        if stale_users:
            logger.debug(
                ...
                len(stale_users), self._tenant_id, presence_key
            )
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tenant_id": self._tenant_id,
            "presence_count": sum(len(v) for v in self._presences.get(self._tenant_id, {}).values()),
        }
