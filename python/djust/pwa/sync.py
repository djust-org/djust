...
class ConflictResolver:
    ...
    def _client_wins(self, local_data: Dict[str, Any], server_data: Dict[str, Any]) -> Dict[str, Any]:
        """Client data takes precedence."""
        result = server_data.copy()
        result.update(local_data)
        return result
    
    def _server_wins(self, local_data: Dict[str, Any], server_data: Dict[str, Any]) -> Dict[str, Any]:
        """Server data takes precedence."""
        return server_data.copy()
    
    def _merge_by_timestamp(self, local_data: Dict[str, Any], server_data: Dict[str, Any]) -> Dict[str, Any]:
...
class SyncManager:
    ...
    def _sync_batch(self, batch: List[OfflineAction], action_type: str, model_name: str) -> Dict[str, Any]:
...
def sync_endpoint_view(request):
    ...
    def sync_view(request):
        ...
        try:
            ...
            logger.info(f"Received sync request with {len(actions_data)} actions from version {version}")
            ...
            response_data = {
                ...
                'synced_ids': [
                    action.id for action in actions[:result.processed_count]
                ],
