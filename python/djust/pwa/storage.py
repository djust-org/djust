...
class IndexedDBStorage(OfflineStorage):
    ...
    def keys(self) -> List[str]:
        ...
        try:
            ...
            return [
                k[len(namespace_prefix):] for k in bridge.keys()
...
class LocalStorage(OfflineStorage):
    ...
    def keys(self) -> List[str]:
        ...
        try:
            namespace_prefix = f"{self._namespace}:"
            return [
                k[len(namespace_prefix):] for k in self._storage.keys()
                if k.startswith(namespace_prefix)
            ]
        except Exception as e:
...
class SyncQueue:
    ...
    def get_pending(self) -> List[OfflineAction]:
        ...
        try:
            queue = self._get_queue()
            return [
                OfflineAction.from_dict(item) for item in queue
    ...
    def remove_completed(self, older_than_hours: int = 24) -> int:
        ...
        try:
            ...
            queue = [
                ...
                if not (item.get('status') == 'completed' and item.get('timestamp', 0) < cutoff_time)
            ]
    ...
    def _update_action_status(self, action_id: str, status: str, error: Optional[str] = None) -> bool:
