...
from .sync import SyncManager, register_sync_handler
...
class PWAMixin:
    ...
    def handle_install_prompt(self):
        ...
        if not is_online():
            self.push_event('pwa:install_offline', {
                'message': 'Install available when online'
            })
            return
        
        self.push_event('pwa:show_install_prompt', {
            'config': self.get_pwa_config()
        })
    
    def handle_app_update(self, version: str):
        """Handle app update notification."""
        self.push_event('pwa:app_update', {
            'version': version,
...
class OfflineMixin:
    ...
    def update_offline(self, model_name: str, obj_id: Union[str, int], data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        """
    ...
    def sync_when_online(self):
        ...
        self.push_event('offline:sync_start', {
...
class SyncMixin:
    ...
    def _process_sync_queue(self):
        ...
        try:
            ...
            self.push_event('offline:sync_complete', {
            ...
            self.push_event('offline:sync_error', {
                'error': str(e)
            })
        finally:
            self._sync_in_progress = False
