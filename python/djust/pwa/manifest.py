    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'description': djust_config.get('PWA_DESCRIPTION', 'Progressive Web App built with djust'),
...
def generate_apple_touch_icon_tags(icons: Optional[List[Dict[str, Any]]] = None) -> List[str]:
    ...
    for icon in icons:
        if 'sizes' in icon:
            tags.append(f'<link rel="apple-touch-icon" sizes="{icon["sizes"]}" href="{icon["src"]}">')
        else:
