...
def test_remove_row_marks_delete_flag():
    ...
    updated = remove_row(AddressFormSet, prefix="form-1", data=data)
...
def test_mixin_add_row_bumps_total_forms():
    ...
    assert v.addresses.data["form-TOTAL_FORMS"] == "1"  # extra=1 initial 0 + 1 added
...
def test_mixin_remove_row_marks_delete():
    ...
    v.remove_row(formset="addresses", prefix="form-0")
    assert v.addresses.data["form-0-DELETE"] == "on"
