    ...
    def update_model(self, field: str = "", value: Any = None, **kwargs):
        ...
        if self.allowed_model_fields is not None:
            if field not in self.allowed_model_fields:
                logger.warning(
                    f"[dj-model] Field '{field}' not in allowed_model_fields"
                )
                return
        ...
        if not hasattr(self, field):
            logger.warning(
                f"[dj-model] Field '{field}' does not exist on "
        ...
        if current is not None and value is not None:
            try:
                ...
                elif isinstance(current, int) and not isinstance(current, bool) and not isinstance(value, int):
                    value = int(value)
