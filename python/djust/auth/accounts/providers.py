"""Display metadata for social sign-in providers (label + static inline SVG icon).

Icons are module constants (never built from input), sized with attributes
and coloured with ``currentColor`` so they follow the theme.
"""
...
_PROVIDER_META = {
    "github": (
        "GitHub",
        '<svg width="18" height="18" aria-hidden="true" fill="currentColor" viewBox="0 0 24 24">'
    ...
    "microsoft": (
        ...
        '<path fill="#00A4EF" d="M1 13h10v10H1z"/><path fill="#FFB900" d="M13 13h10v10H13z"/></svg>',
    ),
}


def icon_for(provider_id: str) -> str:
    """The static SVG icon for a provider id, or ``""`` when there is none."""
    meta = _PROVIDER_META.get(provider_id)
    return meta[1] if meta else ""


def label_for(provider_id: str, default: str) -> str:
    """The display label for a provider id, or ``default``."""
    meta = _PROVIDER_META.get(provider_id)
