...
class LiveViewWebSocket {
            ...
            case 'mount':
                ...
                if (this.skipMountHtml) {
                    ...
                    if (hasDataDjAttrs && data.html) {
                        ...
                        _stampDjIds(data.html); // lgtm[js/xss-through-dom] -- html is server-rendered by the trusted Django/Rust template engine
                    } else {
    ...
    handleEmbeddedUpdate(data) {
        ...
        _morphTemp.innerHTML = html; // lgtm[js/xss-through-dom] -- html is server-rendered by the trusted Django/Rust template engine
...
function _stampDjIds(serverHtml, container) {
    ...
    const parser = new DOMParser();
    const doc = parser.parseFromString('<div>' + serverHtml + '</div>', 'text/html'); // lgtm[js/xss-through-dom] -- serverHtml is rendered by the trusted Django/Rust template engine
