...
from django_rust_live.live_view import cleanup_expired_sessions, get_session_stats, DEFAULT_SESSION_TTL
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--stats',
            action='store_true',
            help='Show session statistics without cleaning up'
        )
    ...
    def handle(self, *args, **options):
        ...
        if not show_stats:
            ...
            if cleaned_count > 0:
                ...
                self.stdout.write(
                    self.style.WARNING('No expired sessions to clean up')
                )
