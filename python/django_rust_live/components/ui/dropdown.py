"""
Dropdown component for Django Rust Live.
...
class DropdownComponent(LiveComponent):
    ...
    def mount(self, **kwargs):
        ...
        self.items = kwargs.get('items', [])  # List of {text, action, icon, variant, divider}
    ...
    def _render_bootstrap(self) -> str:
        ...
        for item in self.items:
            if item.get('divider'):
                ...
                html += f'<li><a class="dropdown-item{variant_class}" href="#"{click_attr}>{content}</a></li>'
    ...
    def _render_tailwind(self) -> str:
        ...
        for item in self.items:
            if item.get('divider'):
                ...
                click_attr = f' @click="{action}; open = false"' if action else ' @click="open = false"'
                variant_class = ' text-red-600' if item_variant == 'danger' else ' text-gray-700'

                content = f'{icon} {text}' if icon else text
                html += f'<a href="#" class="block px-4 py-2 text-sm{variant_class} hover:bg-gray-100"{click_attr}>{content}</a>'
    ...
    def _render_plain(self) -> str:
        ...
        for item in self.items:
            if item.get('divider'):
                ...
                click_attr = f' @click="{action}"' if action else ''
                content = f'{icon} {text}' if icon else text
                html += f'<a href="#" class="dropdown-item"{click_attr}>{content}</a>'
